      ...
      if(!meta)h+='<div class="ac-welcome"><span aria-hidden="true">▧</span><h2>콘텐츠를 골라 살펴보세요</h2><p>그림과 설명을 함께 보고,<br>저장된 페이지와 장면을 차례로 확인할 수 있습니다.</p></div>';
        ...
        if(loading)h+='<p class="ac-empty" role="status">콘텐츠를 불러오는 중…</p>';
          ...
          if(p){var src=imageSource(p.thumb);if(!src&&p.snap&&o.snapPreview){try{src=imageSource(o.snapPreview(p.snap));}catch(_){src='';}}
            h+='<article class="ac-page"><h3>'+esc(p.label||'페이지')+'</h3>'+(src?'<figure class="ac-figure"><img src="'+esc(src)+'" alt="'+esc(p.label||'저장된 장면')+'" referrerpolicy="no-referrer"><figcaption hidden>저장된 그림을 표시하지 못했습니다.</figcaption></figure>':'<div class="ac-no-image">'+(p.snap?'이 페이지에는 배치 데이터가 있지만 표시할 수 있는 그림이 없습니다.':'이 페이지에는 저장된 그림이 없습니다.')+'</div>');
            ...
            h+='<div class="ac-description"><h3>설명·코칭·메모</h3>'+(desc.length?'<dl>'+desc.map(function(r){return '<div><dt>'+esc(r.label)+'</dt><dd>'+esc(r.value)+'</dd></div>';}).join('')+'</dl>':'<p class="ac-muted">저장된 설명이 없습니다.</p>')+'</div></article>';
          }
