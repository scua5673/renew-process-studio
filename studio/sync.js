...
/* 2.675 — 2단계 «열 때 서버부터, 닫으면 지우기»(사용자 "2단계도 이어서 가자").
   ...
   다음 부팅에서 그 표시가 있으면 «팀에서 받는 중…» 덮개를 띄운다. 2.872부터 최대 5초 뒤 작은 수신 안내로 바꿔 화면 전체를 계속 가리지 않는다.
...
var CACHE_WAIT_MS=5000, CACHE_FLAG='ps_cache_wiped_v1', SELF_RELOAD='ps_self_reload', BOOT_AT=Date.now(), cacheWaitTimer=null;
...
function workspacePubLocalKeys(){
  var out=[];
  for(var i=0;i<localStorage.length;i++){var k=localStorage.key(i);if(isIdpPubKey(k))out.push(k);}
  return out;
}
/* Public IDP feedback belongs to a team workspace even though its keys are dynamic.
   A switch must prove that neither localStorage nor an accidental IndexedDB mirror
   still contains one before the destination workspace can sync. */
function verifyWorkspacePubCleared(){
  var local=workspacePubLocalKeys();
  if(local.length)return Promise.reject(new Error('Workspace feedback local wipe verification failed'));
  if(!window.storage)return Promise.resolve(true);
  if(!window.storage.keys)return Promise.reject(new Error('Workspace feedback IndexedDB keys cannot be verified'));
  return Promise.resolve(window.storage.keys()).then(function(keys){
    if(!Array.isArray(keys))throw new Error('Workspace feedback IndexedDB keys cannot be verified');
    var left=keys.filter(isIdpPubKey);
    if(left.length)throw new Error('Workspace feedback IndexedDB wipe verification failed');
    return true;
  });
}
...
function renderCacheWait(mode){
  try{
    ...
    clearTimeout(cacheWaitTimer); cacheWaitTimer=null;
    ...
    if(compact){
      ...
      var rb=ov.querySelector('[data-ps-cw-retry]'); if(rb)rb.onclick=function(){
        ...
        Promise.resolve().then(function(){return syncNow('cache-retry');}).then(function(){
          /* Only the successful round itself can acknowledge CACHE_FLAG. A queued,
             skipped or failed attempt must not leave a new unbounded cover. */
          cacheWaitEnd(false);
        }).catch(function(){cacheWaitEnd(false);});
      };
    } else {
      ov.innerHTML='<div style="'+box+'"><b>팀에서 받는 중…</b><br><span style="color:var(--gray,#646a73)">최신 팀 자료를 확인하고 있어요</span></div>';
...
function cacheWaitEnd(ok){
  if(ok){
    var wasWaiting=cacheWipedFlag()||!!document.getElementById('psCacheWait');
    try{ localStorage.removeItem(CACHE_FLAG); }catch(_){}
    renderCacheWait(null);   /* 2.676 — 리로드하지 않는다. 화면(iframe)은 storage 이벤트로 이미 갱신된다 */
    if(wasWaiting)try{ renderUI(); }catch(_){}
    return;
  }
  /* CACHE_FLAG is shared across tabs, the overlay and timer are not. */
  if(document.getElementById('psCacheWait'))renderCacheWait(navigator.onLine===false?'offline':'slow');
}
...
function conflictRestore(k){
  ...
  return Promise.all(writes).catch(function(e){writeFailed=true;syncDiagnostic('conflict-restore-write',e);}).then(function(){
    ...
    return verify.then(function(cur){
      var mirrorOk=true;if(k===MATCH_KEY)try{mirrorOk=localStorage.getItem(k)===v;}catch(_){mirrorOk=false;}
      if(cur!==v||!mirrorOk){
...
/* 사용자 조회는 요청한 세션에만 적용한다. 늦은 OAuth /user 응답이
   다른 로그인 토큰에 이전 계정의 uid·이메일을 붙이면 캐시 소유자까지 뒤섞인다. */
function sessionIdentitySnapshot(s){
  return s&&s.at&&s.rt?{at:s.at,rt:s.rt,uid:String(s.uid||''),epoch:signOutEpoch}:null;
}
function sessionIdentityCurrent(snapshot){
  var s=getSess();
  return snapshot&&s&&snapshot.epoch===signOutEpoch&&s.at===snapshot.at&&s.rt===snapshot.rt&&
    (!snapshot.uid||String(s.uid||'')===snapshot.uid)?s:null;
}
function sessionIdentityCommit(snapshot,u,email){
...
/* storage 이벤트는 현재 값이 아니라 실제 바뀐 두 세션으로 판정한다.
   A→B→A가 큐에 쌓여도 이전 로그인에서 시작한 작업을 무효화하고,
   같은 uid의 평범한 토큰 갱신은 계속 허용한다. */
function sessionStorageOwnerChanged(e){
  if(!e||e.oldValue===e.newValue)return false;
  var before=null,after=null;
  try{before=JSON.parse(e.oldValue||'null');after=JSON.parse(e.newValue||'null');}catch(_){return true;}
  if(!before||!after)return true;
  return String(before.uid||'')!==String(after.uid||'')||(!before.uid&&before.rt!==after.rt);
}
function ensureSessionIdentity(){
  var s=getSess();if(!s)return Promise.reject(dataLockError());
  if(s.uid)return Promise.resolve(s);
  var epoch=signOutEpoch,request=null;
  return ensureToken().then(function(at){
    var latest=getSess();
    if(epoch!==signOutEpoch||!at||!latest||latest.at!==at)throw dataLockError();
    /* ensureToken의 정상 갱신은 허용하고, 조회를 시작한 뒤의 세션 변경은 막는다. */
    request=sessionIdentitySnapshot(latest);if(!request)throw dataLockError();
    return fetch(BASE+'/auth/v1/user',{headers:hj(at)}).then(function(r){if(!r.ok)throw new Error('user '+r.status);return r.json();});
  }).then(function(u){
    return sessionIdentityCommit(request,u,u&&u.email);
  });
}
function sensitiveLocalKey(k){
  ...
  return k===WSKEY||k===WLKEY||k===MKEY||k===OWNERKEY||k==='ps_sync_outbox_v1'||k==='ps_sync_pending_summary_v1'||k==='ps_sync_skipped'
    ||k==='ps_sync_conflicts_v1'||k==='ps_sync_conf_seen'||k==='ps_hold_list_v1'||k==='ps_rescue_list_v1'||k==='ps_tomb_v1'||k==='ps_personal_review_v1'
    ||k==='ps_roster_keep_v1'||k.indexOf('ps_roster_keep_v2:')===0||k.indexOf('ps_roster_pending_v1:')===0
...
function removeLocalVerified(keys,current){
  keys.forEach(function(k){if(current)current();localStorage.removeItem(k);if(localStorage.getItem(k)!=null)throw new Error(k+' local wipe verify failed');});
}
function idbSensitiveKeys(workspaceOnly){
  ...
  return Promise.resolve(window.storage.keys()).then(function(keys){
    ...
    return keys.filter(function(k){
      if(workspaceOnly)return IDB_CONTENT.indexOf(k)>=0||k.indexOf('sq:')===0||isIdpPubKey(k);
      return sensitiveLocalKey(k)||IDB_CONTENT.indexOf(k)>=0||Object.prototype.hasOwnProperty.call(IDBK,k);
    });
  });
}
function wipeWorkspaceCache(uid,current){
  current();
  setDataReady(false);
  ...
  var local=[];try{CONTENT.concat([MKEY,'ps_sync_outbox_v1','ps_sync_pending_summary_v1']).forEach(function(k){if(local.indexOf(k)<0)local.push(k);});
    ...
    workspacePubLocalKeys().forEach(function(k){if(local.indexOf(k)<0)local.push(k);});
  }catch(e){return Promise.reject(e);}
  return idbSensitiveKeys(true).then(function(keys){
    current();removeLocalVerified(local,current);
    return Promise.all(keys.map(function(k){current();return window.storage.del(k,current);}));
  }).then(function(){current();return verifyWorkspacePubCleared();}).then(function(){current();return true;});
}
function wipeAccountCache(current){
  current();
  setDataReady(false);
  try{syncBaseCacheClear();}catch(_){}
  var local=[];
  try{for(var i=localStorage.length-1;i>=0;i--){var k=localStorage.key(i);if(sensitiveLocalKey(k))local.push(k);}}catch(e){return Promise.reject(e);}
  return idbSensitiveKeys(false).then(function(keys){
    current();removeLocalVerified(local,current);
    return Promise.all(keys.map(function(k){current();return window.storage.del(k,current);}));
  }).then(function(){current();return verifyWorkspacePubCleared();}).then(function(){current();return true;});
}
...
function prepareCacheForSession(s,rows,prepareSwitchEpoch,prepareAuthEpoch){
  rows=Array.isArray(rows)?rows:[];if(!s||!s.uid||!rows.length)throw dataLockError();
  var uid=String(s.uid),own=cacheOwner(),cur=activeWs();
  function prepareCurrent(){
    var live=getSess();
    if(!live||String(live.uid||'')!==uid||signOutEpoch!==prepareAuthEpoch)throw dataLockError();
  ...
  prepareCurrent();setDataReady(false);
  ...
  if(own&&String(own.uid)===uid){
    ...
    return save.then(function(){prepareCurrent();return wipeWorkspaceCache(uid,prepareCurrent);}).then(function(){return finish(defaultWorkspace(rows));});
  }
  if(own&&String(own.uid)!==uid)return wipeAccountCache(prepareCurrent).then(function(){return finish(defaultWorkspace(rows));});
  ...
  return legacyCacheInfo().then(function(info){
    ...
    if(info.uid&&info.uid!==uid)return wipeAccountCache(prepareCurrent).then(function(){return finish(defaultWorkspace(rows));});
    if(!info.uid){
      return askConfirm('로그아웃 전에 이 기기에 저장된 자료가 있습니다.\n현재 로그인한 계정의 자료가 맞습니까?\n\n확인: 현재 계정에 연결\n취소: 이전 자료를 제거하고 서버 자료만 불러오기').then(function(mine){
        prepareCurrent();
        if(!mine)return wipeAccountCache(prepareCurrent).then(function(){return finish(defaultWorkspace(rows));});
...
function loadWorkspacesCore(){
  ...
  function prepareCurrent(){
    var live=getSess();
    if(!live||!verified||String(live.uid||'')!==String(verified.uid||'')||signOutEpoch!==prepareAuthEpoch)throw dataLockError();
  }
  var run=ensureSessionIdentity().then(function(s){
    verified=s;prepareCurrent();
    ...
    return rpc('ps_bootstrap',{p_email:s.email||null},prepareCurrent);
  }).then(function(rows){
    prepareCurrent();
    if(externalSwitchFrozen||String(prepareSwitchEpoch)!==workspaceSwitchEpochRaw()||freezeForPendingExternalSwitch())throw dataLockError();
    return prepareCacheForSession(verified,rows||[],prepareSwitchEpoch,prepareAuthEpoch);
  });
  return run;
}
...
function signIn(provider){
  if(provider!=='google'&&provider!=='kakao'){chip('로그인 방법을 다시 선택해 주세요');return false;}
...
function linkIdentity(provider){
  if(provider!=='google'&&provider!=='kakao'){chip('추가할 로그인 방법을 다시 선택해 주세요');return;}
  var w=window; try{ if(window.top&&window.top!==window&&window.top.location.origin===location.origin)w=window.top; }catch(_){}
  var back=w.location.origin+w.location.pathname;
  var owner=sessionIdentitySnapshot(getSess()),request=null;
  function sameOwner(){var s=getSess();return !!(owner&&owner.uid&&s&&String(s.uid||'')===owner.uid&&signOutEpoch===owner.epoch);}
  if(!sameOwner()){chip('로그인 계정을 확인한 뒤 다시 시도해 주세요');return;}
  ensureToken().then(function(at){
    var current=getSess();
    if(!sameOwner()||!at||!current||current.at!==at)throw dataLockError();
    request=sessionIdentitySnapshot(current);
    return fetch(BASE+'/auth/v1/user/identities/authorize?provider='+encodeURIComponent(provider)+'&redirect_to='+encodeURIComponent(back)+'&skip_http_redirect=true'+(provider==='google'?'&prompt=select_account':''),{headers:hj(at)});
  }).then(function(r){ return r.text().then(function(t){ var j=null; try{ j=JSON.parse(t); }catch(_){}
    /* 연결 URL도 요청한 계정의 응답이다. 로그아웃/다른 로그인 뒤에는 이동하거나
       새 계정 위에 옛 연결 실패 문구를 표시하지 않는다. */
    if(!sessionIdentityCurrent(request))return;
    if(r.ok&&j&&typeof j.url==='string'&&j.url){ w.location.href=j.url; return; }
    ...
    if(request?!sessionIdentityCurrent(request):!sameOwner())return;
    var m='연결 실패 — '+String(e&&e.message||e).slice(0,100); chip(m); setStatus(m);
  });
}
function signOut(){
  ...
  if(!uid||!wid)return Promise.resolve(fail('로그아웃 취소 — 계정과 워크스페이스를 확인하지 못했습니다','prelogout_owner'));
  if(switching)return Promise.resolve(fail('로그아웃 취소 — 워크스페이스 전환이 끝난 뒤 다시 시도하세요','prelogout_switching'));
  ...
  var run=workspaceSwitchWriteBarrier().then(function(gate){
    ...
    return withTimeout(forceSync('prelogout'),15000);
  }).then(function(r){
    if(!sameOwner())throw syncIssue('prelogout_owner','prelogout','account or workspace changed');
    var skipped=Array.isArray(r&&r.skipped)?r.skipped:[],held=Array.isArray(r&&r.held)?r.held:[],deferred=Array.isArray(r&&r.deferred)?r.deferred:[];
    if(!r||r.__timeout||r.error||r.offline||r.noauth||r.nows||r.skip||r.locked||r.itemPending||r.scheduleDeferred||r.serverRejected||skipped.length||held.length||deferred.length){
      var code=String((r&&r.code)||(r&&r.__timeout?'sync_timeout':r&&r.offline?'sync_offline':'prelogout_sync'));
      throw syncIssue(code,'prelogout','prelogout sync not confirmed');
    }
    /* 동기화 중 뒤따라온 입력도 다시 마감한다. 전체화면이 새 입력을 막는 상태라
       이 두 번째 장벽 뒤에는 320ms 저장이 다시 생기지 않는다. */
    return workspaceSwitchWriteBarrier();
  }).then(function(gate2){
    if(!sameOwner())throw syncIssue('prelogout_owner','prelogout','account or workspace changed');
    if(!gate2||gate2.held||gate2.__timeout||gate2.error)throw syncIssue('sync_storage','prelogout','final write barrier failed');
    return withTimeout(logoutBlockingPendingKeys(uid,wid),8000);
  }).then(function(keys){
    if(!sameOwner())throw syncIssue('prelogout_owner','prelogout','account or workspace changed');
    if(keys&&keys.__timeout)throw syncIssue('sync_timeout','prelogout','pending confirmation timeout');
    if(!Array.isArray(keys)||keys.length){
      var labels=Array.isArray(keys)?keys.slice(0,4).map(keyLabel).join('·'):'';
      throw syncIssue('prelogout_pending','prelogout','pending confirmation'+(labels?(': '+labels):''));
    }
    if(overlay)hideSwitchOverlay();return finish();
  }).catch(function(e){
    var info=classifySyncError(e),msg=info.code==='sync_offline'
...
/* 명단 복구는 이 탭이 현재 계정·팀의 동기화를 끝내고, 명단과 삭제 기록이
   그 확정본 그대로일 때만 연다. 과거 localStorage 메타만으로 준비를 추측하지 않는다. */
var rosterSyncMark=null;
function rosterReady(wid){
  try{
    var s=getSess(),own=cacheOwner(),mark=rosterSyncMark,m=meta();
    if(!wid||!mark||!s||!own||!dataUnlocked()||busy||navigator.onLine===false||
       String(wid)!==String(activeWs()||'')||String(mark.wid)!==String(wid)||
       String(mark.uid)!==String(s.uid||'')||String(own.uid||'')!==String(s.uid||'')||
       String(own.wid||'')!==String(wid)||mark.seal!==localStorage.getItem(OWNERKEY)||
       workspaceSwitchGuardRead())return false;
    if(lastIssue&&lastIssue.at>=mark.at)return false;
    if(pendingInfo(wid).count||itemsWriteBusy())return false;
    if(holdList().some(function(x){return x&&(x.k==='scout_tool_v1'||x.k==='cs_player_del_v1');}))return false;
    var held=itemsHoldList();if(held&&held.ids&&held.ids.length)return false;
    return ['scout_tool_v1','cs_player_del_v1'].every(function(k){
      return hash(localStorage.getItem(k))===((m.h||{})[k]||'');
    });
...
/* 경기 준비표는 단순 최종 상태가 아니라 “서버 원문·IDB·거울이
   ...
   기존 표를 먼저 지워, 중간 상태를 scout가 준비 완료로 오해하지 않게 한다.
   setMeta는 저장 실패를 삼키므로 여기서는 되읽기까지 직접 확인한다. */
function matchReadyInvalidate(){
  matchReadyPending=true;
  var before=localStorage.getItem(MKEY),mm;
  try{mm=before==null?{h:{},c:{},last:0,n:{},r:{}}:JSON.parse(before);}
  catch(pe){pe.psCode='match_ready_invalidate';throw pe;}
  if(!mm||typeof mm!=='object'||Array.isArray(mm)){
    var me=new Error('경기 준비표 메타 형식 오류');me.psCode='match_ready_invalidate';throw me;
  }
  if(!mm.r||typeof mm.r!=='object'||Array.isArray(mm.r))mm.r={};
  delete mm.r[MATCH_KEY];
  /* 표가 이미 없다고 읽힌 경우도 exact write/read를 생략하지 않는다. 곧 reload될 때
     디스크에 옛 표가 남는 길을 막고, 읽기/파싱 오류면 다른 meta를 덮지 않고 중단한다. */
  var next=JSON.stringify(mm);
  localStorage.setItem(MKEY,next);
  var stored=localStorage.getItem(MKEY);
  if(stored!==next){var se=new Error('경기 준비표 닫기 저장을 확인하지 못했습니다');se.psCode='match_ready_invalidate';throw se;}
  var chk=JSON.parse(stored||'null');
  if(chk&&chk.r&&Object.prototype.hasOwnProperty.call(chk.r,MATCH_KEY)){
    var e=new Error('경기 준비표를 닫지 못했습니다');e.psCode='match_ready_invalidate';throw e;
  }
  return true;
}
function matchReadyCommit(m,marker){
  if(!marker||typeof marker!=='object')throw new Error('경기 준비표 형식 오류');
  /* 전체 sync meta를 미리 확정하지 않고, 최신 persisted meta의 r 한 칸만 갱신한다.
     그래야 뒤의 library 단계가 실패해도 다른 키의 h/c를 거짓 성공 처리하지 않는다. */
  var latest=meta();latest.r=latest.r||{};latest.r[MATCH_KEY]=marker;
  localStorage.setItem(MKEY,JSON.stringify(latest));
  var chk=JSON.parse(localStorage.getItem(MKEY)||'null'),got=chk&&chk.r&&chk.r[MATCH_KEY];
  if(!got||String(got.w||'')!==String(marker.w||'')||got.present!==marker.present||String(got.h||'')!==String(marker.h||'')){
    var e=new Error('경기 준비표 저장을 확인하지 못했습니다');e.psCode='match_ready_commit';throw e;
  }
  m.r=m.r||{};m.r[MATCH_KEY]=marker;
  matchReadyPending=false;
  return true;
}
...
function idpMergeNeedsReview(merged){
  return !!(merged&&(merged.conflicts||[]).some(function(c){
    /* Two edits often carry different authors/times even when the actual fields
       merge cleanly. Only competing content needs a choice. */
    return c&&!/\.(?:updatedAt|updatedBy|tAt|hAt|ts|at|by|tBy|hBy)$/.test(c.path||'');
  }));
}
...
/* 2.744 — 개인 자료는 팀 화면에서도 저장된다. 전환 장벽의 팀 전용 집계는 그대로
   두고, 사용자에게 보이는 상태만 개인 대기까지 합쳐 “팀과 같아요” 오판을 막는다. */
function visiblePendingInfo(wid){
  var own=pendingInfo(wid),pwid=personalWid(),p=pwid&&pwid!==wid?pendingInfo(pwid):{count:0,oldest:0};
  return {count:own.count+p.count,oldest:own.oldest&&p.oldest?Math.min(own.oldest,p.oldest):(own.oldest||p.oldest)};
}
function outboxMark(wid,key,h,source){
  return outboxMarkForOwner(outboxOwner(),wid,key,h,source);
}
/* storage 이벤트는 값 읽기와 outbox 트랜잭션 사이에 세션이 바뀔 수 있다.
   그 이벤트를 받은 순간의 uid를 명시해 다른 계정의 대기함으로 섞이지 않게 한다. */
function outboxMarkForOwner(uid,wid,key,h,source){
  if(!wid||!key)return Promise.resolve();
  uid=String(uid||'');if(!uid)return Promise.resolve();
  /* Foreign private IDP is a read-only cache. Photo migration can emit a local
     storage event, but that is never an edit this account may publish. Old queue
     entries are retained until a server-confirmed cache projection can ACK them. */
  if(isIdpPrivateKey(key)&&key!==IDP_PRIV_PREFIX+uid)return Promise.resolve();
  var id=outboxScope(uid,wid)+'|'+key,now=Date.now();
...
function outboxMarkRows(wid,rows,m,ownerUid,guard){
  if(!rows||!rows.length)return Promise.resolve();
  if(typeof guard==='function'&&!guard())return Promise.reject(syncIssue('sync_workspace_changed','team_outbox_owner','동기화 대기 등록 중 계정이나 팀이 바뀌었습니다'));
  var uid=ownerUid==null?outboxOwner():String(ownerUid);
  return Promise.all(rows.map(function(r){return outboxMarkForOwner(uid,wid,r.k,(m.h&&m.h[r.k])||hash(r.v),'sync');}));
}
...
function outboxAckSynced(wid,m,skipped,libraryAckHash,guard){
  var uid=outboxOwner(),sc=outboxScope(uid,wid),skip={};
  function requireCurrent(){if(typeof guard==='function'&&!guard())throw syncIssue('sync_workspace_changed','team_ack_owner','팀 저장 확인 중 계정이나 팀이 바뀌었습니다');}
...
function outboxAckPersonal(pwid,m,guard){
  var uid=outboxOwner(),teamWid=isTeamWs()?activeWs():'',confirmed=(m&&m.p&&m.p.h)||{};
  if(!pwid)return Promise.resolve();
  function requireCurrent(){if(typeof guard==='function'&&!guard())throw syncIssue('sync_workspace_changed','personal_ack_owner','개인 저장 확인 중 작업 공간이 바뀌었습니다');}
  return outboxTxn(function(q){
    requireCurrent();
    return Promise.all(q.map(function(it){
...
/* 워크스페이스 전환은 남의 IDP·권한 거부 대기를 제외해 영구 교착을 피하지만,
   로그아웃 뒤 다른 uid가 들어오면 계정 캐시 전체가 지워질 수 있다.
   그러므로 현재 팀 scope와 개인 scope의 이 uid 대기를 하나라도 제외하지 않고 세다. */
function logoutBlockingPendingKeys(uid,wid){
  uid=String(uid||'');wid=String(wid||'');var pwid=String(personalWid()||''),seen={};
  return outboxRead().then(function(q){var keys=[];(q||[]).forEach(function(it){
    if(!it||String(it.uid||'')!==uid||seen[it.key])return;
    var iw=String(it.wid||'');if(iw!==wid&&(!pwid||iw!==pwid))return;
    seen[it.key]=1;keys.push(it.key);
  });return keys;});
}
...
function idbSwitchKeys(){
  ...
  return Promise.resolve(window.storage.keys())
    .then(function(ks){
      ...
      ks.forEach(function(k){ if(isItemKey(k)||CONTENT.indexOf(k)>=0||isIdpPubKey(k))set[k]=1; });
...
function kvPushRowsInner(at,rows,onOk,label,preflight){
  ...
  return cas.reduce(function(p,req){return p.then(function(){
    ready(label+'_cas');
    /* CAS needs a new version even when two saves share a millisecond or a
       device clock trails the server. Leaving cupd unchanged reopens the same
       comparison to a second writer. Callers commit the acknowledged version. */
    if(req._casCupd!=null){
      var previous=Number(req._casCupd),next=Math.max(Number(req.cupd)||0,previous+1);
...
function rescueRestore(k){
  ...
  var writeFailed=false;
  return Promise.all(writes).catch(function(e){writeFailed=true;syncDiagnostic('rescue-restore-write',e);}).then(function(){
    if(!ok||writeFailed) return false;
...
function undoRestore(k){
  ...
  return undoGet(k).then(function(v){
    ...
    var ok=kvWrite(k,v,writes);
    var writeFailed=false;
    return Promise.all(writes).catch(function(e){writeFailed=true;syncDiagnostic('undo-restore-write',e);}).then(function(){
      if(!ok||writeFailed)return false;
      var verify=idbBacked(k)
        ? window.storage.get(k).then(function(r){return r?r.value:null;}).catch(function(){return null;})
        : Promise.resolve((function(){try{return localStorage.getItem(k);}catch(_){return null;}})());
      return verify.then(function(cur){
        var mirrorOk=true;if(k===MATCH_KEY)try{mirrorOk=localStorage.getItem(k)===v;}catch(_){mirrorOk=false;}
        if(cur!==v||!mirrorOk){syncDiagnostic('undo-restore-verify',new Error('restore did not land: '+k));return false;}
...
function holdList(){ if(!dataUnlocked())return [];try{ var s=getSess(),wid=String(activeWs()||'');return (JSON.parse(localStorage.getItem(HOLD_LIST)||'[]')||[]).filter(function(x){return x&&((x.kind!=='conflict'&&x.kind!=='item-delete')||(s&&x.uid===String(s.uid||'')&&x.wid===wid));}); }catch(_){ return []; } }
function holdSetList(a){ try{ var conflicts=a.filter(function(x){return x&&(x.kind==='conflict'||x.kind==='item-delete');}),ordinary=a.filter(function(x){return !x||(x.kind!=='conflict'&&x.kind!=='item-delete');});localStorage.setItem(HOLD_LIST,JSON.stringify(conflicts.concat(ordinary.slice(-HOLD_MAX)))); }catch(_){} }
function holdUnlist(k){ try{ holdSetList(holdList().filter(function(x){ return x.k!==k||x.kind==='conflict'; })); }catch(_){} }
function holdClear(k){ try{ holdUnlist(k); localStorage.removeItem('ps_push_ok_'+k); }catch(_){} }
/* A whole shared document that both coaches changed is never silently replaced.
   This list stores only the reviewed identity/version; the unsent body stays in
   its original local store and the other body stays on the server. */
function holdConflictContext(){
  try{var s=getSess(),wid=String(activeWs()||'');if(!s||!s.uid||!wid||!dataUnlocked())return null;
    return {uid:String(s.uid),wid:wid,seal:String(localStorage.getItem(OWNERKEY)||''),switchSeal:workspaceSwitchGuardRaw(),switchEpoch:workspaceSwitchEpochRaw(),epoch:signOutEpoch};
  }catch(_){return null;}
}
function holdConflictCurrent(ctx){
  var now=holdConflictContext();return !!ctx&&!!now&&ctx.uid===now.uid&&ctx.wid===now.wid&&ctx.seal===now.seal&&ctx.switchSeal===now.switchSeal&&ctx.switchEpoch===now.switchEpoch&&ctx.epoch===now.epoch;
}
function holdConflictSame(a,b){return !!a&&!!b&&a.kind==='conflict'&&b.kind==='conflict'&&a.uid===b.uid&&a.wid===b.wid&&a.k===b.k&&a.h===b.h&&a.c===b.c&&a.sh===b.sh;}
function holdConflictWrite(rows){
  var raw=JSON.stringify(rows);localStorage.setItem(HOLD_LIST,raw);
...
function holdConflictView(x){var ctx=holdConflictContext();return Object.assign({},x,{viewSeal:ctx&&ctx.seal,viewEpoch:ctx&&ctx.switchEpoch,viewSwitch:ctx&&ctx.switchSeal});}
function holdConflictChoose(k,keepMine,shown){
  var ctx=holdConflictContext();if(!ctx||!shown||shown.uid!==ctx.uid||shown.wid!==ctx.wid||shown.k!==k||shown.viewSeal!==ctx.seal||shown.viewEpoch!==ctx.switchEpoch||shown.viewSwitch!==ctx.switchSeal)return Promise.resolve(false);
  return currentValueForKey(k).then(function(loc){
    if(!holdConflictCurrent(ctx)||loc==null||hash(loc)!==shown.h)return false;
    var chosen=false,rows=holdList().map(function(x){if(holdConflictSame(x,shown)){chosen=true;return Object.assign({},x,{choice:keepMine?'local':'server'});}return x;});
    if(!chosen)return false;holdConflictWrite(rows);return {pending:true};
  });
}
function holdConflictClearConfirmed(m,guard){
  var ctx=holdConflictContext();if(!ctx)return Promise.resolve(false);
  var rows=holdList().filter(function(x){return x.kind==='conflict';});
  return Promise.all(rows.map(function(x){return currentValueForKey(x.k).then(function(raw){return {row:x,clear:typeof raw==='string'&&hash(raw)===(m.h||{})[x.k]};});})).then(function(found){
    if(!holdConflictCurrent(ctx)||(typeof guard==='function'&&!guard()))throw syncIssue('sync_workspace_changed','team_review_confirm','팀 자료 확인 중 계정이나 팀이 바뀌었습니다');
...
function holdApprove(k){
  ...
  if(holdList().some(function(x){return x.k===k&&x.kind==='conflict';}))return Promise.resolve(false);
...
function holdTakeServer(k){
  if(holdList().some(function(x){return x.k===k&&x.kind==='conflict';}))return Promise.resolve(false);
  return holdLocal(k).then(function(loc){
    var m=meta(); m.h[k]=hash(loc); m.c[k]=0; delete m.n[k];
    /* 서버본을 아직 받지 않았으므로 옛 로컬 경기를 준비 완료로 열지 않는다. */
    if(k===MATCH_KEY){m.r=m.r||{};delete m.r[k];matchReadyInvalidate();}
    setMeta(m);
    holdClear(k);
    return true;
  });
...
function importSpec(spec){
  ...
  if(s==='players'||s==='scouting'||s==='gamemodel'||s==='match'||s==='match-recovery')out.section=s;
  if(s==='match'){
  ...
  if(s==='match-recovery'){
    var tombSeen={};out.tombstoneIds=(Array.isArray(spec.tombstoneIds)?spec.tombstoneIds:[]).map(function(id){return String(id||'');})
      .filter(function(id){if(!id||tombSeen[id])return false;tombSeen[id]=1;return true;});
    /* 복구 후보는 이 프리플라이트에서 직접 본 서버 판·삭제 기록에만 쓸 수 있다.
       이후 서버가 움직이면 이미 합쳐진 localRaw를 복구 의도로 재사용하지 않는다. */
    out.baseMissing=spec.baseMissing===true;
    out.baseHash=typeof spec.baseHash==='string'?spec.baseHash.slice(0,80):'';
    out.tombstoneHash=typeof spec.tombstoneHash==='string'?spec.tombstoneHash.slice(0,80):'';
    out.tombstoneCupd=Math.max(0,Math.floor(+spec.tombstoneCupd||0));
  }
  return out;
}
...
function importRebaseRaw(k,localRaw,serverRaw,spec){
  ...
  var recoveryMatch=spec.section==='match-recovery'&&k===MATCH_KEY;
  var recognized=recoveryMatch||k==='scout_tool_v1'||(spec.section==='match'&&(k==='cs_match_del_v1'||k==='cs_team_match_private_v1'||k.indexOf('cs_idp_pub_v1_')===0));
  try{
    if(recoveryMatch){
      /* 이 경로의 localRaw는 이미 base+backup이 합쳐진 결과다. 최신 서버 위에 다시
         merge하면 그 사이 다른 기기가 지운 경기·비운 필드를 되살린다. 따라서 서버와
         삭제 기록이 승인 당시 exact 기준 그대로일 때만 동일 후보의 CAS를 허용한다. */
      var liveRaw=null;try{liveRaw=localStorage.getItem(MATCH_DEL_KEY);}catch(e){throw new Error('삭제한 경기 기록을 읽지 못했습니다');}
      recoveryDeletedIds(liveRaw); /* null만 빈 기록이며, 손상/빈 문자열은 fail closed */
      if(hash(liveRaw)!==spec.tombstoneHash)return null;
      if(serverRaw==null)return spec.baseMissing?localRaw:null;
      if(spec.baseMissing||typeof serverRaw!=='string'||!spec.baseHash||hash(serverRaw)!==spec.baseHash)return null;
      return localRaw;
    }
...
function workspaceHoldReviewPrepare(keys,from,resumeWid,seq){
  ...
  return Promise.all(keys.map(function(k){return holdLocal(k);})).then(function(raws){
    ...
    keys.forEach(function(k,i){
      ...
      if(raw!=null&&h&&h.h&&hash(raw)===h.h)rows.push(Object.assign(holdConflictView(h),{raw:raw}));
    });
...
function holdTakeServerExact(k,expectedRaw,expectedHash){
  ...
  return holdLocal(k).then(function(loc){
    ...
    var m=meta();m.h[k]=hash(loc);m.c[k]=0;delete m.n[k];
    if(k===MATCH_KEY){m.r=m.r||{};delete m.r[k];matchReadyInvalidate();}
    setMeta(m);holdClear(k);return true;
  });
}
...
function workspaceHoldReviewOpen(opts){
  ...
  function choose(k,keepMine){
    ...
    workspaceHoldReviewExact(intent,row).then(function(exact){
      ...
      if(row.kind==='conflict')return holdConflictChoose(row.k,keepMine,row);
...
function holdOpen(opts){
  ...
  if(list.some(function(x){return x.kind==='conflict';}))return dataReviewOpen();
...
function dataReviewList(){
  ...
  var out=[];
  try{personalReviewList().forEach(function(x){out.push(Object.assign({},x,{src:'personal',mine:null,theirs:null,
    why:x.choice?'선택을 기록했습니다 · 두 판본을 다시 확인한 뒤 반영합니다':'개인 자료가 두 기기에서 바뀌어 양쪽 내용을 그대로 보관 중입니다'}));});}catch(_){}
  ...
  var rank={personal:0,team:0,'item-delete':0,hold:1,rescue:2,conflict:3}, seen={}, uniq=[];
  out.sort(function(a,b){ return (rank[a.src]-rank[b.src])||(b.at-a.at); });
  out.forEach(function(x){ if(!seen[x.k]){ seen[x.k]=1; uniq.push(x); } });
  return uniq;
}
function dataReviewApply(src,k,keepMine,shown){
  if(src==='item-delete')return itemsDeleteReviewChoose(shown,keepMine);
  if(src==='personal')return personalReviewChoose(k,keepMine,shown);
  if(src==='team')return holdConflictChoose(k,keepMine,shown);
  var run;
...
function histLoad(k,current){
  var wid=activeWs(); if(!wid) return Promise.resolve([]);
  return rpc('ps_admin_kv_history',{p_wid:wid,p_key:k,p_limit:30},current)
    .then(function(rows){ if(current)current();return Array.isArray(rows)?rows:[]; })
    .catch(function(){ return null; });          /* null = 못 불러옴(권한·서버) */
}
/* Read-only incident comparison. Historical membership is matched by exact ID;
   current player fields and every original raw string remain untouched. */
function histRosterDoc(raw){
  var d=typeof raw==='string'&&JSON.parse(raw),seen=new Set();
  if(!d||typeof d!=='object'||Array.isArray(d)||!Array.isArray(d.players)||!Array.isArray(d.attrs)||!Array.isArray(d.positions))throw new Error('명단 원문의 형식을 확인하지 못했습니다.');
  d.players.forEach(function(p){if(!p||typeof p!=='object'||Array.isArray(p)||typeof p.id!=='string'||!p.id.trim()||p.id!==p.id.trim()||seen.has(p.id))throw new Error('비어 있거나 중복된 선수 ID가 있어 비교를 중단했습니다.');seen.add(p.id);});
  return d;
}
function histRosterCompare(currentRaw,baselineRaw,permsRaw,deletedRaw,itemRows){
  var cur=histRosterDoc(currentRaw),base=histRosterDoc(baselineRaw),perms=JSON.parse(permsRaw||'{"members":{}}'),deleted=JSON.parse(deletedRaw||'{}');
  function object(x){return !!x&&typeof x==='object'&&!Array.isArray(x);}
  if(!object(perms)||(perms.members!=null&&!object(perms.members))||!object(deleted))throw new Error('계정 연결 또는 삭제 기록을 확인하지 못했습니다.');
  Object.keys(deleted).forEach(function(id){if(!Number.isFinite(deleted[id])||deleted[id]<=0)throw new Error('선수 삭제 기록의 형식을 확인하지 못했습니다.');});
  function ours(p){return p.type!=='target'&&!p._del;}
  function canonical(v){return Array.isArray(v)?v.map(canonical):object(v)?Object.keys(v).sort().map(function(k){return [k,canonical(v[k])];}):v;}
  function same(a,b){return JSON.stringify(canonical(a))===JSON.stringify(canonical(b));}
  var current=new Map(cur.players.filter(ours).map(function(p){return [p.id,p];})),baseline=new Map(base.players.filter(ours).map(function(p){return [p.id,p];})),items=new Map(),links=new Map();
  if(!Array.isArray(itemRows))throw new Error('선수별 저장 원문을 확인하지 못했습니다.');
  itemRows.forEach(function(r){
    if(!r||typeof r.k!=='string'||r.k.indexOf('sq:')!==0||!r.k.slice(3)||items.has(r.k.slice(3))||typeof r.raw!=='string')throw new Error('선수별 저장 ID를 확인하지 못했습니다.');
    var p=JSON.parse(r.raw),id=r.k.slice(3);if(!object(p)||(p._del?(!Number.isFinite(p._del)||p._del<=0):p.id!==id))throw new Error('선수별 저장 원문과 ID가 다릅니다.');items.set(id,p);
  });
  Object.keys(perms.members||{}).forEach(function(uid){var m=perms.members[uid],id=m&&m.playerId;if(typeof id==='string'&&id)links.set(id,(links.get(id)||0)+1);});
  var common=[],extra=[],missing=[];
  current.forEach(function(p,id){(baseline.has(id)?common:extra).push(id);});
  baseline.forEach(function(p,id){if(!current.has(id))missing.push(id);});
  var rows=extra.concat(missing,common).map(function(id){
    var p=current.get(id)||baseline.get(id),present=current.has(id),b=baseline.get(id),it=items.get(id),records=[];
    [['memo','메모'],['memos','메모 목록'],['levels','평가값'],['evalSets','평가 기록'],['meetings','면담 기록']].forEach(function(pair){var v=p[pair[0]];if(Array.isArray(v)?v.length:object(v)?Object.keys(v).length:String(v==null?'':v).trim())records.push(pair[1]);});
    return {id:id,name:String(p.name||''),grp:String(p.grp||''),status:!present?'baseline-only':b?'common':'current-only',changed:!!(present&&b&&!same(p,b)),linkedAccounts:links.get(id)||0,records:records,item:!it?'none':it._del?'deleted':'live',deleted:Object.prototype.hasOwnProperty.call(deleted,id)};
  });
  return {currentCount:current.size,baselineCount:baseline.size,commonIds:common,extraIds:extra,missingIds:missing,baselineIsSubset:missing.length===0,rows:rows,outsideBaseline:extra.map(function(id){return current.get(id);})};
}
function histRosterContext(){
  try{var ctx=holdConflictContext(),role=window.PSPerms&&window.PSPerms.role&&window.PSPerms.role(),w=activeWsObj();
    if(!ctx||!w||w.kind!=='team'||['admin','executive','staff'].indexOf(role)<0)return null;
    ctx.histPerms=localStorage.getItem('cs_perms_v1');ctx.histRole=role;ctx.histWsRole=String(w.role||'');return ctx;
  }catch(_){return null;}
}
function histRosterCurrent(ctx){var now=histRosterContext();return !!now&&holdConflictCurrent(ctx)&&!workspaceSwitchGuardRead()&&!externalSwitchFrozen&&ctx.histPerms===now.histPerms&&ctx.histRole===now.histRole&&ctx.histWsRole===now.histWsRole;}
function histReadView(host,ctx){
  var closed=false,timer=null,events=['ps-auth-state','ps-sync-state','storage'];
  function stop(){if(closed)return;closed=true;clearInterval(timer);events.forEach(function(e){window.removeEventListener(e,check);});host.querySelectorAll('textarea').forEach(function(t){t.value='';});host.textContent='';}
  function current(){if(closed)return false;if(host.isConnected===false||!histRosterCurrent(ctx)){stop();return false;}return true;}
  function requireCurrent(){if(!current())throw new Error('계정이나 팀이 바뀌어 비교 내용을 닫았습니다.');}
  function check(e){
    if(e&&((e.type==='ps-auth-state'&&e.detail&&e.detail.unlocked===false)||(e.type==='storage'&&(e.key===null||([OWNERKEY,WSKEY,WLKEY,WS_SWITCH_GUARD,WS_SWITCH_EPOCH,'cs_perms_v1'].indexOf(e.key)>=0&&e.oldValue!==e.newValue)||(e.key===SKEY&&sessionStorageOwnerChanged(e)))))){stop();return;}
    current();
  }
  events.forEach(function(e){window.addEventListener(e,check);});timer=setInterval(check,250);current();
  return {current:current,require:requireCurrent,stop:stop};
}
function histRosterSnapshot(row,ctx,current){
  function guard(){current();if(!histRosterCurrent(ctx))throw new Error('계정이나 팀이 바뀌어 비교를 중단했습니다.');}
  function local(){guard();return {raw:localStorage.getItem('scout_tool_v1'),deleted:localStorage.getItem('cs_player_del_v1'),perms:localStorage.getItem('cs_perms_v1')};}
  function readItems(){
    guard();if(!window.storage||!window.storage.keys||!window.storage.get)throw new Error('선수별 저장소를 읽을 수 없습니다.');
    return window.storage.keys().then(function(keys){guard();return Promise.all(keys.filter(function(k){return typeof k==='string'&&k.indexOf('sq:')===0;}).sort().map(function(k){return window.storage.get(k).then(function(r){guard();if(!r||typeof r.value!=='string')throw new Error('선수별 원문이 읽는 동안 바뀌었습니다. 다시 비교해 주세요.');return {k:k,raw:r.value};});}));});
  }
  return Promise.resolve().then(function(){
    guard();if(!row||typeof row.v!=='string'||!row.v.trim())throw new Error('선택한 판본의 원문을 확인하지 못했습니다.');
    var start=local();return readItems().then(function(items){return readItems().then(function(check){
      guard();if(JSON.stringify(start)!==JSON.stringify(local())||JSON.stringify(items)!==JSON.stringify(check))throw new Error('읽는 동안 명단이 바뀌었습니다. 다시 비교해 주세요.');
      var comparison=histRosterCompare(start.raw,row.v,start.perms,start.deleted,items);
      return {type:'process-studio-roster-inspection',schema:1,workspaceId:ctx.wid,capturedAt:new Date().toISOString(),baselineAt:row.at||row.changed_at||row.ts||null,
        raw:{scout_tool_v1:start.raw,baseline_scout_tool_v1:row.v,cs_player_del_v1:start.deleted},items:items,comparison:comparison};
    });});
  });
}
function histRosterShow(host,snapshot,view,bcss){
  view.require();var c=snapshot.comparison;
  host.innerHTML='<div style="margin:14px 0 7px;font-size:14px;font-weight:800">명단 비교 · 읽기 전용</div>'
    +'<div>현재 기기 <b>'+c.currentCount+'명</b> · 선택 판본 <b>'+c.baselineCount+'명</b><br>같은 ID '+c.commonIds.length+'명 · 현재에만 '+c.extraIds.length+'명 · 판본에만 '+c.missingIds.length+'명</div>'
    +'<div style="font-size:11px;opacity:.75;margin:7px 0">이름과 조는 참고로 표시합니다. 같은 ID는 현재 기기의 정보를 보여 줍니다. 이 화면은 선수 명단이나 기록을 바꾸지 않습니다. 계정 연결 수와 선수 원문 안의 기록만 확인하며 개인 IDP 본문은 읽지 않습니다.</div>'
    +'<div style="overflow:auto"><table style="border-collapse:collapse;table-layout:fixed;width:100%;font-size:11px"><thead><tr><th>구분 · ID</th><th>이름 · 조</th><th>연결 · 기록</th></tr></thead><tbody>'
    +c.rows.map(function(r){return '<tr><td style="border-top:1px solid #8884;padding:7px;vertical-align:top;overflow-wrap:anywhere">'+(r.status==='current-only'?'현재에만':r.status==='baseline-only'?'판본에만':'같은 ID'+(r.changed?' · 내용 변경':''))+'<br>'+esc(r.id)+'</td><td style="border-top:1px solid #8884;padding:7px;vertical-align:top;overflow-wrap:anywhere">'+esc(r.name||'이름 없음')+'<br>'+esc(r.grp||'조 없음')+'</td><td style="border-top:1px solid #8884;padding:7px;vertical-align:top;overflow-wrap:anywhere">계정 '+r.linkedAccounts+'개<br>'+esc(r.records.join(' · ')||'기록 필드 없음')+'<br>선수별 저장 '+(r.item==='live'?'있음':r.item==='deleted'?'삭제 표식':'없음')+(r.deleted?' · 명단 삭제 표식':'')+'</td></tr>';}).join('')+'</tbody></table></div>'
    +'<div style="font-size:11px;opacity:.75;margin:9px 0">백업에는 두 명단 원문, 현재 기기의 선수별 저장 원문과 삭제 기록이 들어갑니다. 별도로 저장된 사진 원본과 다른 메뉴 자료는 포함하지 않습니다.</div>'
    +'<button type="button" data-hist-download style="'+bcss+'">비교·원문 백업 파일 저장</button>'
    +'<details style="margin-top:8px"><summary>백업 원문 펼치기</summary><textarea data-hist-backup aria-label="명단 비교와 원문 백업" readonly style="width:100%;box-sizing:border-box;height:180px;font:11px monospace;margin-top:6px"></textarea></details>';
  var json=JSON.stringify(snapshot,null,2),area=host.querySelector('[data-hist-backup]');area.value=json;
  host.querySelector('[data-hist-download]').addEventListener('click',function(){
    if(!view.current())return;
    var url=URL.createObjectURL(new Blob([json],{type:'application/json'})),a=document.createElement('a');
    a.href=url;a.download='process-studio-roster-inspection-'+Date.now()+'.json';host.appendChild(a);a.click();a.remove();setTimeout(function(){URL.revokeObjectURL(url);},2000);
  });
}
...
function dataReviewOpen(){
  ...
  if(legacyExport)legacyExport.addEventListener('click',function(){try{if(!itemsLostExport(legacyItems))chip('계정이나 사본이 바뀌었습니다. 자료 확인을 다시 열어 주세요.');}catch(e){syncDiagnostic('items-legacy-export',e);chip('충돌 사본을 내보내지 못했습니다. 사본은 그대로 남아 있습니다.');}});
  (function(){
  ...
  (function(){
    ...
    var view=null,compareRun=0;
    lb.addEventListener('click',function(){
      if(view)view.stop();
      var k=sel.value,ctx=histRosterContext();view=histReadView(out,ctx);var shown=view;
      if(!shown.current())return;
      lb.disabled=true; lb.textContent='불러오는 중…'; out.innerHTML='';
      histLoad(k,shown.require).then(function(rows){
        if(!shown.current())return;
        ...
        out.innerHTML=rows.map(function(r,i){
          ...
          return '<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;'
            ...
            +(available?'<div style="display:flex;gap:5px;flex-wrap:wrap">'+(k==='scout_tool_v1'?'<button type="button" data-hist-compare="'+i+'" style="'+bcss+'">명단 비교·백업 보기</button>':'')+'<button type="button" data-hist="'+i+'" style="'+bcss+'">되돌리기</button></div>':'<button type="button" disabled style="'+bcss+';opacity:.5">복구 불가</button>')+'</div>';
        }).join('');
        var preview=document.createElement('div');out.appendChild(preview);
        out.querySelectorAll('[data-hist-compare]').forEach(function(cb){
          cb.addEventListener('click',function(){
            if(!shown.current())return;
            var run=++compareRun,r=rows[+cb.getAttribute('data-hist-compare')];preview.textContent='명단과 선수별 원문을 읽는 중…';
            histRosterSnapshot(r,ctx,shown.require).then(function(snapshot){
              if(shown.current()&&run===compareRun)histRosterShow(preview,snapshot,shown,bcss);
            }).catch(function(){if(shown.current()&&run===compareRun)preview.textContent='비교 원문을 확인하지 못했습니다. 읽는 중 자료가 바뀌었거나 원문 형식·저장소에 문제가 있습니다. 명단은 바꾸지 않았습니다. 다시 비교해 주세요.';});
          });
        });
  ...
  document.querySelectorAll('[data-dr-delete-export]').forEach(function(b){b.addEventListener('click',function(){itemsDeleteReviewExport(list[+b.getAttribute('data-dr-delete-export')]);});});
  document.querySelectorAll('[data-dr]').forEach(function(b){
    b.addEventListener('click',function(){
      ...
      Promise.resolve(dataReviewApply(x.src,x.k,keep,x)).then(function(ok){
        try{ if(mo&&mo.close)mo.close(true); }catch(_){}
        if(ok===false){
        ...
        try{ renderUI(); }catch(_){}
        if(x.src==='item-delete'){
          try{psModal({title:keep?'삭제를 이 기기에 저장했습니다':'추가 삭제를 취소했습니다',body:keep?'팀 반영 여부는 동기화 상태에서 확인하세요.':'이미 저장된 삭제 기록은 되돌리지 않습니다. 바뀐 선수 원문은 그대로 유지합니다.',hideCancel:true,ok:'확인'});}catch(_){}
          return;
        }
        if(ok&&ok.pending){
          try{psModal({title:'선택을 기록했습니다',body:'동기화에서 두 판본이 그대로인지 확인한 뒤 '+(keep?'이 기기 내용을 올립니다.':'다른 기기 내용을 받습니다.')+' 그동안 내용이 바뀌면 다시 확인할 수 있게 남겨 둡니다.',hideCancel:true,ok:'확인'});}catch(_){}
          return;
        }
        ...
        b.disabled=false;b.textContent=keep?'이 기기 것':'다른 기기 것';
        try{psModal({title:'선택을 저장하지 못했습니다',body:'자료는 그대로 두었습니다. 저장 공간을 확인한 뒤 다시 시도해 주세요.',hideCancel:true,ok:'확인'});}catch(_){}
      });
      }
    });
  });
...
var ITEMS_SRV_TTL=864e5,itemsServerPromise=null;
...
function itemsServerCheck(){
  var s=itemsSrvState(),age=s?Date.now()-(+s.at||0):Infinity;
  /* A failed request proves no server capability. The old six-hour negative
     cache stranded all roster writes after a brief network/auth interruption. */
  var ttl=s&&s.ok?ITEMS_SRV_TTL:(s&&s.err?30000:300000);
  if(s&&age>=0&&age<ttl&&(!s.err||s.v===2))return Promise.resolve(!!s.ok);
  if(itemsServerPromise)return itemsServerPromise;
  itemsServerPromise=Promise.resolve().then(function(){return rpc('ps_key_scope',{key:ITEMP+'__probe'});}).then(function(v){
    var ok=(String(v||'')==='team');
    try{ localStorage.setItem(ITEMS_SRV,JSON.stringify({v:2,ok:ok?1:0,at:Date.now(),saw:String(v||'')})); }catch(_){}
    if(!ok) syncDiagnostic('items-server-old',new Error('ps_key_scope='+v+' — supabase-item-keys.sql 미설치'));
    return ok;
  }).catch(function(e){
    /* Remain closed, but check again on the next retry after 30 seconds. */
    try{ localStorage.setItem(ITEMS_SRV,JSON.stringify({v:2,ok:0,at:Date.now(),err:1})); }catch(_){}
    syncDiagnostic('items-server-check',e); return false;
  }).then(function(ok){itemsServerPromise=null;return ok;},function(e){itemsServerPromise=null;throw e;});
  return itemsServerPromise;
}
...
var _itemsWritingN=0,_itemsActiveJobs=[],_itemsObserved=null,_itemsExplicitFailures=[],_itemsPatchFailures=[],_itemsJobSequence=0;
function itemsWriteBusy(){return !!(_itemsPending||_itemsWritingN||_itemsExplicitFailures.some(itemsWriteRetryable)||_itemsPatchFailures.some(itemsWriteRetryable)||itemsWriteRetryable(_itemsFailedJob));}
function itemsWriteRetryable(job){return !!job&&(job.patch?job.changedIds.length+job.deletedIds.length>0:(!job.explicit||job.deletedIds.length>0))&&itemsWriteCurrent(job.owner)&&(!job.patch||itemsPatchPermission()===job.permission)&&(!job.reviewContext||histRosterCurrent(job.reviewContext))&&(!job.intent||itemsDeleteCurrent(job.intent));}
function itemsReviewKeys(){
  var keys={};if(typeof holdList==='function')holdList().forEach(function(x){if(x&&x.kind==='item-delete'&&x.itemKey)keys[x.itemKey]=1;else if(x&&x.kind==='conflict'&&x.k&&x.k.indexOf(ITEMP)===0)keys[x.k]=1;});return keys;
}
function itemsDeleteReviews(){return typeof holdList==='function'?holdList().filter(function(x){return x&&x.kind==='item-delete';}):[];}
function itemsDeleteReviewLabel(rec){var name='';[rec.current,rec.expected].some(function(raw){try{var p=JSON.parse(raw);name=String(p&&p.name||'').trim();}catch(_){}return !!name;});return '선수 삭제 확인 · '+(name||'이름 없음')+' ('+rec.itemKey.slice(ITEMP.length)+')';}
function itemsDeleteReviewView(rec){var ctx=histRosterContext();return ctx?Object.assign(holdConflictView(rec),{reviewContext:ctx}):null;}
function itemsDeleteReviewShown(shown){
  var ctx=holdConflictContext();if(!ctx||!shown||!histRosterCurrent(shown.reviewContext)||shown.uid!==ctx.uid||shown.wid!==ctx.wid||shown.viewSeal!==ctx.seal||shown.viewEpoch!==ctx.switchEpoch||shown.viewSwitch!==ctx.switchSeal)return null;
  return itemsDeleteReviews().find(function(x){return x.k===shown.k&&x.at===shown.at&&x.expected===shown.expected&&x.current===shown.current&&x.tomb===shown.tomb;})||null;
}
function itemsDeleteReviewExport(shown){
  var rec=itemsDeleteReviewShown(shown);if(!rec)return false;
  var url=URL.createObjectURL(new Blob([JSON.stringify(rec,null,2)],{type:'application/json'})),a=document.createElement('a');a.href=url;a.download='process-player-delete-review.json';a.click();setTimeout(function(){URL.revokeObjectURL(url);},1000);return true;
}
function itemsDeleteReviewChoose(shown,remove){
  var rec=itemsDeleteReviewShown(shown),owner=itemsWriteOwner();if(!rec||!owner)return Promise.resolve(false);
  if(!scoutWriteAllowed())return Promise.resolve(false);
  if(!remove){holdConflictWrite(holdList().filter(function(x){return x.k!==rec.k;}));return Promise.resolve(true);}
  var id=rec.itemKey.slice(ITEMP.length),job=itemsWriteJob([],owner,{deletedIds:[id]});job.reviewContext=shown.reviewContext;
  job.ready=job.ready.then(function(){
    if(!itemsDeleteReviewShown(shown)||!itemsWriteCurrent(owner))throw syncIssue('sync_workspace_changed','items_delete_review','삭제 확인 중 계정이나 팀이 바뀌었습니다');
    if(job.expected[id]!==rec.current){
      holdConflictWrite(holdList().map(function(x){return x.k===rec.k?Object.assign({},x,{current:job.expected[id]}):x;}));
      throw syncIssue('sync_local_changed','items_delete_review','선수 원문이 다시 바뀌었습니다. 자료 확인을 다시 열어 주세요.');
    }
  });
  job.ready.catch(function(){});itemsWriteDrain().catch(function(){});
  return itemsWriteEnqueue(job).then(function(result){return !!result&&!result.held&&!result.skipped;});
}
/* A stale delete is a review, not a global upload barrier. Persist both the
   request-time original and the changed row before releasing its failed job. */
function itemsWriteReview(job){
  return Promise.all(job.deletedIds.map(function(id){return window.storage.get(ITEMP+id).then(function(r){return {id:id,raw:r&&r.value!=null?r.value:null};});})).then(function(rows){
    if(!itemsWriteCurrent(job.owner)||(job.patch&&itemsPatchPermission()!==job.permission))throw syncIssue('sync_workspace_changed','items_delete_review','선수 삭제 확인 중 계정이나 팀이 바뀌었습니다');
    var reviews=holdList(),tomb=JSON.stringify({_del:job.at});
    rows.forEach(function(row){
      if(row.raw===tomb)return;
      var k='@item-delete:'+row.id,old=reviews.find(function(x){return x.k===k;});
      if(old&&old.at>job.at)return;
      reviews=reviews.filter(function(x){return x.k!==k;});
      reviews.push({kind:'item-delete',k:k,itemKey:ITEMP+row.id,uid:job.owner.uid,wid:job.owner.wid,at:old?old.at:job.at,expected:old?old.expected:job.expected[row.id],current:row.raw,tomb:old?old.tomb:tomb});
    });
    holdConflictWrite(reviews);return {held:true,review:rows.length,gone:0};
  });
}
function itemsDeleteComplete(job){
  if(!job.deletedIds||!job.deletedIds.length)return;
  var ids=job.deletedIds.slice(),reviews=itemsDeleteReviews(),done=reviews.filter(function(x){return ids.indexOf(x.itemKey.slice(ITEMP.length))>=0&&x.at<=job.at;});
  if(done.length)holdConflictWrite(holdList().filter(function(x){return !done.some(function(r){return r.k===x.k&&r.at===x.at&&r.current===x.current&&r.expected===x.expected&&r.tomb===x.tomb;});}));
  _itemsExplicitFailures.forEach(function(old){if(old!==job&&old.at<=job.at&&itemsWriteCurrent(old.owner))old.deletedIds=old.deletedIds.filter(function(id){return ids.indexOf(id)<0;});});
  _itemsExplicitFailures=_itemsExplicitFailures.filter(function(old){return old.deletedIds.length>0;});
}
/* A newer confirmed edit supersedes only the same IDs in older failed patches.
   Distinct edits retain their own exact preimages, including after partial I/O. */
function itemsPatchComplete(job){
  var ids=(job.patch?job.changedIds:[]).concat(job.deletedIds||[],job.reviewedDeletes||[]);
  if(!ids.length)return;
  _itemsPatchFailures.forEach(function(old){
    if(old===job||old.sequence>=job.sequence||!itemsWriteCurrent(old.owner))return;
    old.changedIds=old.changedIds.filter(function(id){return ids.indexOf(id)<0;});
    old.deletedIds=old.deletedIds.filter(function(id){return ids.indexOf(id)<0;});
  });
  _itemsPatchFailures=_itemsPatchFailures.filter(function(old){return old.changedIds.length+old.deletedIds.length>0;});
}
function itemsObserveServer(srv){
  var owner=itemsWriteOwner(),versions={};if(!owner||!itemsWriteCurrent(owner))return;
  Object.keys(srv||{}).forEach(function(k){if(k.indexOf(ITEMP)===0&&srv[k]&&Number.isFinite(srv[k].cupd))versions[k]=srv[k].cupd;});
  var changed=!_itemsObserved||!itemsWriteCurrent(_itemsObserved.owner)||JSON.stringify(_itemsObserved.versions)!==JSON.stringify(versions);
  _itemsObserved={owner:owner,versions:versions};return changed;
}
...
function itemsCreateAllowed(k,raw){
  ...
  try{p=JSON.parse(raw);if(p&&p._del)return true;
    doc=JSON.parse(localStorage.getItem('scout_tool_v1')||'null');deleted=JSON.parse(localStorage.getItem('cs_player_del_v1')||'{}');
  }catch(_){return false;}
  /* Old local sq remnants are not an explicit roster addition. Only a player
     already in the current document may create a previously absent server row. */
  return !!p&&String(p.id||'')===id&&!(deleted&&deleted[id])&&!!doc&&Array.isArray(doc.players)&&doc.players.some(function(x){return x&&String(x.id||'')===id&&x.type!=='target';});
}
function itemsWriteOwner(){
  ...
  return {uid:String(ss.uid),wid:String(activeWs()),seal:localStorage.getItem(OWNERKEY),epoch:workspaceSwitchEpochRaw(),signout:signOutEpoch};
}
function itemsWriteCurrent(owner){
  if(!owner)return false;
  var now=itemsWriteOwner();
  return !!now&&now.uid===owner.uid&&now.wid===owner.wid&&now.seal===owner.seal&&now.epoch===owner.epoch&&now.signout===owner.signout&&!workspaceSwitchGuardRead();
}
function itemsWriteSnapshot(players){
  if(!Array.isArray(players))throw syncIssue('sync_storage','items_write_input','선수 명단 형식을 확인할 수 없습니다');
...
function itemsWriteJob(players,owner,options){
  if(!owner)throw syncIssue('sync_workspace_changed','items_write_owner','선수 명단의 계정과 팀을 확인할 수 없습니다');
  var job={players:itemsWriteSnapshot(players),owner:owner,at:Date.now(),sequence:++_itemsJobSequence,written:[],predecessors:_itemsActiveJobs.slice(),expected:{},strict:!!(options&&Object.prototype.hasOwnProperty.call(options,'expectedRows')),patch:!!(options&&Object.prototype.hasOwnProperty.call(options,'changedIds')),changedIds:[]};
  job.deletedIds=[];
  if(options&&options.deletedIds!=null){
    if(!Array.isArray(options.deletedIds))throw syncIssue('sync_storage','items_delete_ids','삭제할 선수 ID를 확인할 수 없습니다');
    options.deletedIds.forEach(function(id){if(typeof id!=='string'||!id||job.deletedIds.indexOf(id)>=0||job.players.some(function(p){return p&&String(p.id)===id;}))throw syncIssue('sync_storage','items_delete_ids','삭제할 선수와 남길 명단이 다릅니다');job.deletedIds.push(id);});
  }
  if(job.patch){
    job.permission=itemsPatchPermission();
    if(job.permission===null)throw syncIssue('sync_workspace_changed','items_patch_permission','선수 명단을 저장할 권한을 확인할 수 없습니다');
    /* A prior patch may have committed this row before another row or its
       index failed. Its proven CAS transitions remain valid predecessors. */
    _itemsPatchFailures.forEach(function(old){if(job.predecessors.indexOf(old)<0)job.predecessors.push(old);});
    if(job.strict||!Array.isArray(options.changedIds)||!Array.isArray(options.basePlayers))throw syncIssue('sync_storage','items_patch_ids','변경한 선수와 편집 기준을 확인할 수 없습니다');
    function patchId(id){return typeof id==='string'&&id.trim()===id&&!!id&&['__proto__','constructor','prototype'].indexOf(id)<0;}
    if(job.deletedIds.some(function(id){return !patchId(id);}))throw syncIssue('sync_storage','items_patch_ids','삭제할 선수 ID를 확인할 수 없습니다');
    options.changedIds.forEach(function(id){
      var matches=job.players.filter(function(p){return p&&String(p.id)===id;});
      if(!patchId(id)||job.changedIds.indexOf(id)>=0||job.deletedIds.indexOf(id)>=0||matches.length!==1||!String(matches[0].name||'').trim()||matches[0].type==='target'||matches[0]._del)throw syncIssue('sync_storage','items_patch_ids','변경한 선수와 저장할 명단이 다릅니다');
      job.changedIds.push(id);
    });
    var bases=Object.create(null);
    itemsWriteSnapshot(options.basePlayers).forEach(function(p){
      if(!p||!patchId(p.id)||Object.prototype.hasOwnProperty.call(bases,p.id))throw syncIssue('sync_storage','items_patch_base','선수 편집 기준의 ID를 확인할 수 없습니다');
      bases[p.id]=itemVal(p);
    });
    job.changedIds.concat(job.deletedIds).forEach(function(id){job.expected[id]=Object.prototype.hasOwnProperty.call(bases,id)?bases[id]:null;});
    job.history=Object.create(null);
    if(options.baseHistory!=null){
      if(!Array.isArray(options.baseHistory))throw syncIssue('sync_storage','items_patch_history','보관한 선수 변경 이력을 확인할 수 없습니다');
      options.baseHistory.forEach(function(row){
        if(!row||!patchId(row.id)||!Object.prototype.hasOwnProperty.call(job.expected,row.id)||Object.prototype.hasOwnProperty.call(job.history,row.id)||!Array.isArray(row.players))throw syncIssue('sync_storage','items_patch_history','보관한 선수 변경 이력의 ID가 다릅니다');
        job.history[row.id]=itemsWriteSnapshot(row.players).map(function(p){
          if(!p||Array.isArray(p)||p.id!==row.id||p._del||p.type==='target')throw syncIssue('sync_storage','items_patch_history','보관한 선수 변경 이력의 원문이 다릅니다');
          return itemVal(p);
        });
      });
    }
  }
  job.explicit=job.deletedIds.length>0;
  var ids=Object.create(null),seen={},idx=itemsIdx();
  if(job.patch)job.changedIds.forEach(function(id){ids[id]=1;});
  else{job.players.forEach(function(p){if(p&&p.id&&String(p.name||'').trim()&&p.type!=='target'){var id=String(p.id);seen[id]=1;if(idx[id]!==hash(itemVal(p)))ids[id]=1;}});
    Object.keys(idx).forEach(function(id){if(!seen[id])ids[id]=1;});}
  /* A deletion retry must never replay its old whole-roster snapshot over a
     later edit or a second confirmed deletion. Its only writes are these IDs. */
  if(job.deletedIds.length&&!job.patch)ids=Object.create(null);
  job.deletedIds.forEach(function(id){ids[id]=1;});
  if(job.patch){
    /* The displayed/outbox baseline is the only predecessor the edit knew.
       Reading a newer IDB row now cannot authorize overwriting its fields. */
    job.ready=Promise.resolve();
  }else if(job.strict){
    if(!Array.isArray(options.expectedRows))throw syncIssue('sync_storage','items_write_snapshot','선수 복구 기준을 확인할 수 없습니다');
    options.expectedRows.forEach(function(row){var id=String(row&&row.id||'');if(!id||!row||typeof row.raw!=='string'||Object.prototype.hasOwnProperty.call(job.expected,id))throw syncIssue('sync_storage','items_write_snapshot','선수 복구 기준 형식이 다릅니다');job.expected[id]=row.raw;});
    Object.keys(ids).forEach(function(id){if(!Object.prototype.hasOwnProperty.call(job.expected,id))job.expected[id]=null;});
    job.ready=Promise.resolve();
  }else{
    /* Start reading before the 1.2s debounce. A retry keeps these exact
       preimages; it must never rebase an old whole-player snapshot onto a new
       server/other-tab row merely because its later get() succeeds. */
    job.ready=Promise.all(Object.keys(ids).map(function(id){
      if(!window.storage||!window.storage.get)return Promise.reject(syncIssue('sync_storage','items_write_snapshot','선수 저장 기준을 읽을 수 없습니다'));
      return window.storage.get(ITEMP+id).then(function(r){if(!itemsWriteCurrent(owner))throw syncIssue('sync_workspace_changed','items_write_snapshot','선수 저장 기준을 읽는 동안 계정이나 팀이 바뀌었습니다');job.expected[id]=r&&r.value!=null?r.value:null;});
    }));
  }
  job.ready.catch(function(){});_itemsActiveJobs.push(job);return job;
}
function itemsWritePreimage(job,id,raw){
  var known=Object.prototype.hasOwnProperty.call(job.expected,id),value=known?job.expected[id]:null,allowed=known?[value]:[];
  /* Owner-scoped durable pending records may retain earlier authored values
     after a reload loses the in-memory CAS chain. Never adopt a fresh read. */
  if(job.patch&&job.history&&job.history[id])allowed=allowed.concat(job.history[id]);
  var earlier=job.strict?[]:job.predecessors.filter(function(p){return p.owner&&job.owner&&p.owner.seal===job.owner.seal&&p.owner.epoch===job.owner.epoch;});
  earlier.concat([job]).forEach(function(p){(p.written||[]).forEach(function(w){if(w.id!==id)return;
    if(!known&&!allowed.length){allowed.push(w.before);known=true;}
    if(allowed.indexOf(w.before)>=0&&allowed.indexOf(w.after)<0)allowed.push(w.after);
  });});
  return allowed.indexOf(raw)>=0;
}
function itemsWriteCheckSnapshot(job){
  if(!job.strict)return Promise.resolve(true);
  if(!window.storage.keys)return Promise.reject(syncIssue('sync_storage','items_write_snapshot','선수 행 목록을 확인할 수 없습니다'));
  var expected=Object.assign({},job.expected);job.written.forEach(function(w){expected[w.id]=w.after;});
  return window.storage.keys().then(function(keys){
    if(!itemsWriteCurrent(job.owner))throw syncIssue('sync_workspace_changed','items_write_snapshot','선수 복구 확인 중 팀이 바뀌었습니다');
    var actual=keys.filter(function(k){return k.indexOf(ITEMP)===0;}).map(function(k){return k.slice(ITEMP.length);}).sort();
    var wanted=Object.keys(expected).filter(function(id){return expected[id]!=null;}).sort();
    if(JSON.stringify(actual)!==JSON.stringify(wanted))throw syncIssue('sync_local_changed','items_write_snapshot','선수 행 목록이 바뀌었습니다. 복구를 다시 확인해 주세요.');
    return Promise.all(Object.keys(expected).map(function(id){return window.storage.get(ITEMP+id).then(function(r){if((r&&r.value!=null?r.value:null)!==expected[id])throw syncIssue('sync_local_changed','items_write_snapshot','선수 정보가 바뀌었습니다. 복구를 다시 확인해 주세요.');});}));
  });
}
function itemsWriteEnqueue(job){
  _itemsWritingN++;
  function finished(){_itemsWritingN--;_itemsActiveJobs=_itemsActiveJobs.filter(function(x){return x!==job;});}
  var run=_itemsWriteTail.catch(function(){}).then(function(){return job.ready;}).then(function(){return job.rows?itemsWriteRowsNow(job):itemsWriteNow(job.players,job.owner,job);}).catch(function(e){
    if(job.deletedIds&&job.deletedIds.length&&e.psCode==='sync_local_changed'&&itemsWriteCurrent(job.owner)&&(!job.patch||e.psItemDeleted))return itemsWriteReview(job).then(function(value){
      if(!job.patch)return value;
      /* Durable review owns these deletion requests now. It must not hide a
         failed live edit in the same patch or block unrelated later edits. */
      job.reviewedDeletes=(job.reviewedDeletes||[]).concat(job.deletedIds);job.deletedIds=[];
      return itemsWriteNow(job.players,job.owner,job);
    });
    throw e;
  }).then(function(value){
    if(value&&value.skipped==='server')throw syncIssue('sync_confirm_missing','items_write_server','선수별 서버 저장 준비를 확인하지 못했습니다. 연결이 돌아오면 다시 확인합니다.');
    if(job.patch&&(!value||value.skipped))throw syncIssue('sync_confirm_missing','items_patch_unconfirmed','변경한 선수의 저장을 확인하지 못했습니다');
    if(job.patch&&value&&value.held)throw syncIssue('sync_storage','items_patch_unconfirmed','변경한 선수의 저장 확인이 남아 있습니다');
    if(value&&job.reviewedDeletes&&job.reviewedDeletes.length)value.reviewedDeletes=job.reviewedDeletes.slice();
    if(value&&!value.held&&!value.skipped){itemsDeleteComplete(job);itemsPatchComplete(job);}return value;
  }).then(function(value){finished();return value;},function(e){finished();throw e;});
  _itemsWriteTail=run.then(function(value){if(!job.patch||_itemsFailedJob===job)_itemsFailedJob=null;_itemsExplicitFailures=_itemsExplicitFailures.filter(function(x){return x!==job;});_itemsPatchFailures=_itemsPatchFailures.filter(function(x){return x!==job;});return value;},function(e){
    if(itemsWriteRetryable(job)){
      if(job.patch){if(_itemsPatchFailures.indexOf(job)<0)_itemsPatchFailures.push(job);}
      else if(job.deletedIds&&job.deletedIds.length){if(_itemsExplicitFailures.indexOf(job)<0)_itemsExplicitFailures.push(job);}
      else _itemsFailedJob=job;
    }
    syncDiagnostic('items-write',e);
  });
  return run;
}
function itemsWriteDrain(){
  clearTimeout(_itemsT);_itemsT=null;
  var job=_itemsPending,waiters=_itemsWaiters;_itemsPending=null;_itemsWaiters=[];
  if(!job)return _itemsWriteTail;
  var run=itemsWriteEnqueue(job);
  run.then(function(value){waiters.forEach(function(w){w.resolve(value);});},function(e){waiters.forEach(function(w){w.reject(e);});});
  return run;
}
/* 입력은 모아서 쓰되 요청 시점의 계정·팀과 자료를 고정한다. 지연 중 팀이 바뀌면
   예전 명단을 새 팀의 sq: 행으로 저장하지 않고, 실패한 행의 색인도 전진하지 않는다. */
function itemsWrite(players,options){
  if(!ITEMS_ACTIVE)return Promise.resolve({skipped:'disabled'});
  var owner=itemsWriteOwner();if(!owner)return Promise.resolve({skipped:'owner'});
  var job;try{job=itemsWriteJob(players,owner,options);}catch(e){syncDiagnostic('items-write',e);return Promise.resolve(null);}
  /* An explicit confirmation is not a debounced roster snapshot. Imported rows
     may be absent from the index, and the next ordinary save must not erase it. */
  if(job.patch||job.deletedIds.length){itemsWriteDrain().catch(function(){});return itemsWriteEnqueue(job).catch(function(e){syncDiagnostic('items-write',e);return null;});}
  if(_itemsPending&&(_itemsPending.owner.seal!==owner.seal||_itemsPending.owner.epoch!==owner.epoch))itemsWriteDrain().catch(function(){});
  if(_itemsPending){var replaced=_itemsPending;_itemsActiveJobs=_itemsActiveJobs.filter(function(x){return x!==replaced;});job.predecessors=job.predecessors.filter(function(x){return x!==replaced;});}
  _itemsPending=job;
  var result=new Promise(function(resolve,reject){_itemsWaiters.push({resolve:resolve,reject:reject});});
  if(!_itemsT)_itemsT=setTimeout(function(){itemsWriteDrain().catch(function(){});},1200);
  /* 기존 자동 저장 호출부와 호환. 엄격한 완료가 필요한 복구는 writeReady를 사용한다. */
  return result.catch(function(e){syncDiagnostic('items-write',e);return null;});
}
function itemsWriteReady(players,options){
  var job;try{job=itemsWriteJob(players,itemsWriteOwner(),options);}catch(e){return Promise.reject(e);}
  if(!job.owner)return Promise.reject(syncIssue('sync_workspace_changed','items_write_owner','선수 명단의 계정과 팀을 확인할 수 없습니다'));
  itemsWriteDrain().catch(function(){});
  return itemsWriteEnqueue(job).then(function(result){
    if(!result||result.skipped||result.held)throw syncIssue('sync_storage','items_write_unconfirmed','선수 명단 저장 확인이 끝나지 않았습니다');
    return result;
  });
}
function itemsWriteFlush(){
  var tail=_itemsPending?itemsWriteDrain():_itemsWriteTail;
  return tail.then(function(){
    _itemsExplicitFailures=_itemsExplicitFailures.filter(itemsWriteRetryable);
    return _itemsExplicitFailures.slice().reduce(function(p,job){return p.then(function(){return _itemsExplicitFailures.indexOf(job)>=0&&itemsWriteRetryable(job)?itemsWriteEnqueue(job):true;});},Promise.resolve());
  }).then(function(){
    _itemsPatchFailures=_itemsPatchFailures.filter(itemsWriteRetryable);
    return _itemsPatchFailures.slice().reduce(function(p,job){return p.then(function(){return _itemsPatchFailures.indexOf(job)>=0&&itemsWriteRetryable(job)?itemsWriteEnqueue(job):true;});},Promise.resolve());
  }).then(function(){
...
/* Older teams may have a main-document player but no per-player row yet.
   Missing local cache or a truncated roster list cannot prove remote absence.
   Probe only this exact key; the later server INSERT still uses ignore/CAS. */
function itemsWriteConfirmAbsent(job,row,current){
  return Promise.resolve().then(function(){current();return ensureToken();}).then(function(at){
    current();if(!at)throw syncIssue('sync_confirm_missing','items_patch_absence','선수별 저장 기준을 확인할 로그인이 없습니다');
    var url=BASE+'/rest/v1/ps_kv?workspace_id=eq.'+encodeURIComponent(job.owner.wid)+'&k=eq.'+encodeURIComponent(row.k)+'&select=k,v,cupd&limit=2';
    return syncFetch('items_patch_absence',url,{headers:hj(at)});
  }).then(function(r){current();if(!r.ok)throw syncHttpError('items_patch_absence',r.status);return r.json();}).then(function(rows){
    current();if(!Array.isArray(rows))throw syncIssue('sync_confirm_missing','items_patch_absence','선수별 저장 기준 응답을 확인하지 못했습니다');
    return rows.length===0;
  });
}
function itemsWriteNow(players,owner,job){
  if(job&&(job.patch?!job.changedIds.length&&!job.deletedIds.length:job.explicit&&!job.deletedIds.length)){
    if(!itemsWriteCurrent(job.owner)||(job.patch&&itemsPatchPermission()!==job.permission))return Promise.reject(syncIssue('sync_workspace_changed','items_write_owner','선수 저장 중 계정·팀 또는 권한이 바뀌었습니다'));
    return Promise.resolve({superseded:true});
  }
  if(!ITEMS_ACTIVE)return Promise.resolve({skipped:'disabled'});
  if(!window.storage||!window.storage.get||!window.storage.replaceIfValue)return Promise.reject(syncIssue('sync_storage','items_write_storage','안전한 선수 저장소를 사용할 수 없습니다'));
  if(!Array.isArray(players))return Promise.reject(syncIssue('sync_storage','items_write_input','선수 명단 형식 오류'));
  if(localStorage.getItem('ps_items_write')==='0')return Promise.resolve({skipped:'disabled'});
  owner=owner||itemsWriteOwner();
  if(!job){try{job=itemsWriteJob(players,owner);}catch(e){return Promise.reject(e);}return itemsWriteEnqueue(job);}
  function requireOwner(){if(!itemsWriteCurrent(owner)||(job.patch&&itemsPatchPermission()!==job.permission)||(job.reviewContext&&(!histRosterCurrent(job.reviewContext)||!scoutWriteAllowed())))throw syncIssue('sync_workspace_changed','items_write_owner','선수 저장 중 계정·팀 또는 권한이 바뀌었습니다');}
  function rowChanged(stage,message,row){var e=syncIssue('sync_local_changed',stage,message);e.psItemId=row.k.slice(ITEMP.length);e.psItemDeleted=!!row.explicit;return e;}
  return Promise.resolve().then(function(){requireOwner();return itemsServerCheck();}).then(function(ready){
    requireOwner();if(!ready)return {skipped:'server'};
    var idx=itemsIdx(),now=job.at,seen={},next={},rows=[],wrote=0,review=itemsReviewKeys(),reviewHeld={},deleted={},explicit=!!(job.deletedIds&&job.deletedIds.length),changed=Object.create(null);
    if(explicit||job.patch)next=Object.assign({},idx);
    job.changedIds.forEach(function(id){changed[id]=1;});
    (job.deletedIds||[]).forEach(function(id){deleted[id]=1;});
    players.forEach(function(p){
      if(!p||!p.id||!String(p.name||'').trim()||p.type==='target')return;
      var id=String(p.id),v=itemVal(p);if(v==null)return;
      if(seen[id])throw syncIssue('sync_storage','items_write_duplicate','선수 식별자가 중복되어 저장을 멈췄습니다');
      seen[id]=1;
      if(job.patch?!changed[id]:explicit)return;
      /* A whole-roster projection is not a decision about an unresolved row.
         Keep its exact local review original and its previous index entry. */
      if(review[ITEMP+id]){reviewHeld[id]=1;if(Object.prototype.hasOwnProperty.call(idx,id))next[id]=idx[id];return;}
      next[id]=hash(v);
      if(job.patch||idx[id]!==next[id]){rows.push({k:ITEMP+id,v:v});wrote++;}
    });
    if(!explicit&&!job.patch)Object.keys(idx).forEach(function(id){if(review[ITEMP+id]){next[id]=idx[id];reviewHeld[id]=1;}});
    var gone=(explicit||job.patch)?[]:Object.keys(idx).filter(function(id){return !seen[id]&&!review[ITEMP+id];}),held=!!(gone.length&&bigDrop(Object.keys(idx).filter(function(id){return !review[ITEMP+id];}).length,Object.keys(seen).filter(function(id){return !review[ITEMP+id];}).length));
    if(held){
      var hold={at:now,ids:gone.slice(),before:Object.keys(idx).length,after:Object.keys(seen).length,uid:owner.uid,wid:owner.wid,hashes:{}};
      gone.forEach(function(id){hold.hashes[id]=hash(job.expected[id]);});
      localStorage.setItem(ITEMS_HOLD,JSON.stringify(hold));
      gone.forEach(function(id){next[id]=idx[id];});
    }else gone.forEach(function(id){rows.push({k:ITEMP+id,v:JSON.stringify({_del:now})});});
    Object.keys(deleted).forEach(function(id){delete next[id];rows.push({k:ITEMP+id,v:JSON.stringify({_del:now}),explicit:true});});
    return itemsWriteCheckSnapshot(job).then(function(){return Promise.all(rows.map(function(row){return window.storage.get(row.k).then(function(before){
      requireOwner();var raw=before&&before.value!=null?before.value:null,id=row.k.slice(ITEMP.length);
      if(raw!==row.v&&!itemsWritePreimage(job,id,raw)){
        if(job.patch&&raw===null&&typeof job.expected[id]==='string')return itemsWriteConfirmAbsent(job,row,requireOwner).then(function(absent){
          requireOwner();if(!absent)throw rowChanged('items_write_stale_job','선수별 서버 원문이 있어 최신 명단 확인이 필요합니다.',row);
          job.expected[id]=null;row.expected=null;
        });
        throw rowChanged('items_write_stale_job','입력 뒤 선수 정보가 바뀌었습니다. 최신 명단과 내 입력을 확인해 주세요.',row);
      }
      row.expected=raw;
    });}));}).then(function(){
    /* Every CAS uses the validated request-time preimage, never a fresh value
       adopted after the debounce. Own earlier queued batches may advance it. */
    return rows.reduce(function(chain,row){return chain.then(function(){
      requireOwner();return Promise.resolve().then(function(){
        if(!row.explicit&&itemsReviewKeys()[row.k]){var id=row.k.slice(ITEMP.length);reviewHeld[id]=1;if(Object.prototype.hasOwnProperty.call(idx,id))next[id]=idx[id];else delete next[id];row.review=true;return;}
        if(row.expected===row.v)return;
        return window.storage.replaceIfValue(row.k,row.expected,row.v,job.patch?requireOwner:undefined).then(function(ok){
          if(!ok)throw rowChanged('items_write_cas','선수 정보에 더 새로운 변경이 있습니다',row);
          job.written.push({id:row.k.slice(ITEMP.length),before:row.expected,after:row.v});
          if(!itemsWriteCurrent(owner))return window.storage.replaceIfValue(row.k,row.v,null).then(function(){requireOwner();});
        });
      }).then(function(){
        if(row.review)return;
        requireOwner();return window.storage.get(row.k).then(function(check){
          requireOwner();if(!check||check.value!==row.v)throw syncIssue('sync_storage','items_write_verify','선수 저장 내용을 확인하지 못했습니다');
        });
      });
    });},Promise.resolve()).then(function(){return itemsWriteCheckSnapshot(job);}).then(function(){
      requireOwner();
      if(job.patch){
        /* Another tab may advance an unrelated index entry while these row
           transactions wait. Only this patch's verified IDs may change it. */
        var latest=Object.assign({},itemsIdx());
        job.changedIds.forEach(function(id){if(!reviewHeld[id]&&Object.prototype.hasOwnProperty.call(next,id))latest[id]=next[id];});
        job.deletedIds.forEach(function(id){delete latest[id];});next=latest;
      }
      var key=itemsIdxKey(),raw=JSON.stringify(next);
      localStorage.setItem(key,raw);if(localStorage.getItem(key)!==raw)throw syncIssue('sync_storage','items_write_index','선수 저장 색인을 확인하지 못했습니다');
      return {wrote:wrote,gone:gone.length+Object.keys(deleted).length,held:held||Object.keys(reviewHeld).length>0,review:Object.keys(reviewHeld).length,n:Object.keys(seen).length};
    });
    });
  });
}
/* "정말 지운 게 맞습니다" — 보류해 둔 삭제를 실행한다 */
function itemsDeleteIntent(){
  var owner=itemsWriteOwner(),raw=localStorage.getItem(ITEMS_HOLD),hold=raw&&JSON.parse(raw);
  if(!owner||!itemsWriteCurrent(owner)||!hold||!Array.isArray(hold.ids)||!hold.ids.length)return null;
  if((hold.uid&&hold.uid!==owner.uid)||(hold.wid&&hold.wid!==owner.wid))return null;
  return {owner:owner,raw:raw,hold:hold};
}
...
function itemsApproveDel(intent){
  try{
    intent=intent||itemsDeleteIntent();
    if(!itemsDeleteCurrent(intent))throw syncIssue('sync_workspace_changed','items_delete_owner','삭제를 확인하는 동안 계정·팀 또는 명단이 바뀌었습니다');
    if(!window.storage||!window.storage.get||!window.storage.replaceIfValue)throw syncIssue('sync_storage','items_delete_storage','안전한 선수 저장소를 사용할 수 없습니다');
    if(_itemsFailedJob&&_itemsFailedJob.intent&&_itemsFailedJob.intent.raw===intent.raw&&itemsWriteCurrent(_itemsFailedJob.owner))
      return itemsWriteEnqueue(_itemsFailedJob).then(function(result){if(itemsWriteCurrent(intent.owner))try{syncNow('items-del');}catch(_){}return result.gone;});
    /* The persisted hold timestamp identifies this approval's tombstones even
       after a reload loses the in-memory retry job. Never adopt other deletions. */
    var ids=intent.hold.ids.map(String),seen={},now=+intent.hold.at;
    if(!isFinite(now)||now<=0)throw syncIssue('sync_storage','items_delete_stamp','삭제 보류 시점을 확인하지 못했습니다. 최신 명단을 확인해 주세요.');
    var tomb=JSON.stringify({_del:now});
    var job={owner:intent.owner,at:now,sequence:++_itemsJobSequence,rows:[],written:[],intent:intent,ready:null};
    ids.forEach(function(id){if(!id||seen[id])throw syncIssue('sync_storage','items_delete_ids','삭제할 선수 목록을 확인하지 못했습니다');seen[id]=1;});
    job.ready=Promise.all(ids.map(function(id){return window.storage.get(ITEMP+id).then(function(r){
      if(!itemsDeleteCurrent(intent))throw syncIssue('sync_workspace_changed','items_delete_owner','삭제를 확인하는 동안 계정·팀 또는 명단이 바뀌었습니다');
      var raw=r&&r.value!=null?r.value:null,hashes=intent.hold.hashes;
      if(raw!==tomb&&hashes&&Object.prototype.hasOwnProperty.call(hashes,id)&&hash(raw)!==hashes[id])throw syncIssue('sync_local_changed','items_delete_stale','삭제 보류 뒤 선수 정보가 바뀌었습니다. 최신 명단을 확인해 주세요.');
      job.rows.push({k:ITEMP+id,expected:raw,v:tomb});
    });}));
    job.ready.catch(function(){});_itemsActiveJobs.push(job);itemsWriteDrain().catch(function(){});
    return itemsWriteEnqueue(job).then(function(result){
      if(!result||result.skipped)throw syncIssue('sync_confirm_missing','items_delete_unconfirmed','선수 삭제를 저장하지 못했습니다');
      if(itemsWriteCurrent(intent.owner))try{syncNow('items-del');}catch(_){}
      return result.gone;
    });
  }catch(e){return Promise.reject(e);}
}
function itemsWriteRowsNow(job){
  function current(){return itemsDeleteCurrent(job.intent);}
  function requireCurrent(){if(!current())throw syncIssue('sync_workspace_changed','items_delete_owner','선수 삭제 중 계정·팀 또는 명단이 바뀌었습니다');}
  return Promise.resolve().then(function(){requireCurrent();if(!ITEMS_ACTIVE||localStorage.getItem('ps_items_write')==='0')throw syncIssue('sync_confirm_missing','items_delete_disabled','선수별 저장이 준비되지 않았습니다');return itemsServerCheck();}).then(function(ready){
    requireCurrent();if(!ready)return {skipped:'server'};
    return job.rows.reduce(function(chain,row){return chain.then(function(){
      requireCurrent();var wrote=job.written.some(function(x){return x.k===row.k;});
      return window.storage.replaceIfValue(row.k,wrote?row.v:row.expected,row.v).then(function(ok){
        if(!ok)throw syncIssue('sync_local_changed','items_delete_cas','선수 정보에 더 새로운 변경이 있습니다');
        if(!wrote)job.written.push({k:row.k});
        if(!itemsWriteCurrent(job.owner))return window.storage.replaceIfValue(row.k,row.v,null).then(function(){requireCurrent();});
        requireCurrent();return window.storage.get(row.k).then(function(check){requireCurrent();if(!check||check.value!==row.v)throw syncIssue('sync_storage','items_delete_verify','선수 삭제 저장을 확인하지 못했습니다');});
      });
    });},Promise.resolve());
  }).then(function(result){
    if(result&&result.skipped)return result;
    requireCurrent();var idx=itemsIdx(),key=itemsIdxKey();job.rows.forEach(function(row){delete idx[row.k.slice(ITEMP.length)];});
    var raw=JSON.stringify(idx);localStorage.setItem(key,raw);if(localStorage.getItem(key)!==raw)throw syncIssue('sync_storage','items_delete_index','선수 삭제 색인을 확인하지 못했습니다');
    requireCurrent();localStorage.removeItem(ITEMS_HOLD);if(localStorage.getItem(ITEMS_HOLD)!==null)throw syncIssue('sync_storage','items_delete_hold','삭제 확인 상태를 저장하지 못했습니다');
    return {gone:job.rows.length};
  });
}
function itemsHoldOpen(){
  var h=itemsHoldList();
  if(!h||!h.ids||!h.ids.length){ try{ psModal({title:'보류된 삭제가 없습니다',hideCancel:true,ok:'확인'}); }catch(_){} return; }
  var intent=null;try{intent=itemsDeleteIntent();}catch(_){}if(!intent)return;
  var bcss='border:1px solid rgba(128,128,128,.4);background:transparent;color:inherit;font:inherit;font-size:12px;padding:6px 10px;border-radius:8px;cursor:pointer';
  var body='<div style="margin-bottom:10px">명단이 <b>'+h.before+'명 → '+h.after+'명</b>으로 줄었습니다. '
    +'빠진 '+h.ids.length+'명의 <b>삭제 확인이 남아 있습니다.</b><br>'
    +'맞게 정리한 것이면 아래에서 확인해 주세요. 저장 도중 멈췄다면 이어서 처리합니다. '
    +'추가 삭제를 멈춰도 이미 저장된 삭제는 되돌리지 않습니다.</div>'
    +'<div style="display:flex;gap:6px;flex-wrap:wrap">'
    +'<button type="button" id="itDelOk" style="'+bcss+'">맞게 지웠습니다</button>'
    +'<button type="button" id="itDelNo" style="'+bcss+'">추가 삭제 안 하기</button></div>';
  ...
  if(ok)ok.addEventListener('click',function(){ok.disabled=true;itemsApproveDel(intent).then(function(n){
...
function itemsReadAll(){
  ...
  var owner=itemsWriteOwner();if(!owner||!itemsWriteCurrent(owner))return Promise.resolve(null);
  return Promise.resolve(window.storage.keys()).then(function(ks){
    ...
    if(!itemsWriteCurrent(owner))return null;
    var players=[], tombs={}, n=0, rows=0,confirmed={},m=meta(),observed=_itemsObserved&&itemsWriteCurrent(_itemsObserved.owner)?_itemsObserved.versions:{};
    pairs.forEach(function(pr){ var k=pr[0], v=pr[1]; if(v==null)return; rows++; var o; try{ o=JSON.parse(v); }catch(_){ return; }
      ...
      if(Object.prototype.hasOwnProperty.call(observed,k)&&m.c[k]===observed[k]&&m.h[k]===hash(v))confirmed[k]=1;
    });
    return {players:players,tombs:tombs,n:n,rows:rows,held:itemsReviewKeys(),confirmed:confirmed,pending:itemsWriteBusy()};
  }).catch(function(e){ syncDiagnostic('items-read',e); return null; });
}
/* Legacy copies have no workspace identity. Keep them intact and offer an
   exact file export in data review; never apply them to the current team. */
var _itemConflicts=[];
function itemsLostSnapshot(){
  var owner=itemsWriteOwner();if(!owner||!itemsWriteCurrent(owner))return null;
  var raw=localStorage.getItem('ps_items_lost_v1'),rows=raw&&JSON.parse(raw);
  return Array.isArray(rows)&&rows.length?{owner:owner,raw:raw,n:rows.length}:null;
}
function itemsLostExport(snapshot){
  if(!snapshot||!itemsWriteCurrent(snapshot.owner)||localStorage.getItem('ps_items_lost_v1')!==snapshot.raw)return false;
  var url=URL.createObjectURL(new Blob([snapshot.raw],{type:'application/json'})),a=document.createElement('a');
  a.href=url;a.download='process-roster-conflict-copies.json';a.click();setTimeout(function(){URL.revokeObjectURL(url);},1000);return true;
}
/* A roster CAS miss must preserve its unsent local row. Persist an exact review
   of the local/server pair and stop before any cache or metadata replacement. */
function itemsResolveConflicts(at,wid,ownerGuard,roundMeta){
  ...
  return syncFetch('items_conflict',BASE+'/rest/v1/ps_kv?workspace_id=eq.'+encodeURIComponent(wid)+'&'+kvInFilter(ks)+'&select=k,v,cupd',{headers:hj(at)})
    ...
    .then(function(rows){
      ...
      var m=roundMeta||meta(),planned=[],conflicts=[];
      return Promise.all(reqs.map(function(req){
        ...
        return currentValueForKey(req.k).then(function(raw){
          if(!ownerCurrent())throw syncIssue('sync_workspace_changed','items_conflict_apply','선수 행 확인 중 작업 공간이 바뀌었습니다');
          if(typeof raw!=='string')throw syncIssue('sync_conflict','items_conflict_local_stale','선수 기기 원문을 확인하지 못했습니다');
          holdConflictRecord(req.k,raw,x);conflicts.push(req.k);
        });
      })).then(function(){
        if(!ownerCurrent())throw syncIssue('sync_workspace_changed','items_conflict_commit','선수 행 적용 중 작업 공간이 바뀌었습니다');
        if(conflicts.length)throw syncIssue('sync_conflict','items_conflict_review','선수 정보가 두 기기에서 바뀌었습니다. 자료 확인에서 남길 판본을 골라 주세요.');
...
function matchMirrorWriteExact(v){
  if(typeof v!=='string')throw new Error('경기 원문 형식 오류');
  localStorage.setItem(MATCH_KEY,v);
        ...
        .then(function(r){
          ...
          if(expectedLoc!==undefined){try{mirror=localStorage.getItem(k);mirrorReadable=true;}catch(_){}
            ...
            /* A previous partial apply can already have put the exact candidate
               ...
               additions that retain every name from both local copies. */
...
/* ── 개인 채널(1.502) ────────────────────────────────────────────────
   ...
   - 2.744: 개인 공간에서도 이 통로를 쓴다. 본 KEYS 루프의 무조건 쓰기로 우회하지 않는다. */
function personalWid(){
  try{
    ...
    return mine.length?(mine[0].id||''):'';   /* 2.744 — 명시적으로 다른 소유자인 공간으로 폴백하지 않는다. */
  }catch(_){ return ''; }
}
...
function personalContext(){
  ...
  return {uid:String(s.uid),wid:String(wid),active:String(activeWs()||''),epoch:signOutEpoch,
    seal:String(localStorage.getItem(OWNERKEY)||''),switchSeal:workspaceSwitchGuardRaw(),switchEpoch:workspaceSwitchEpochRaw()};
}
function personalCurrent(ctx,guard){
  try{var s=getSess();return !!ctx&&dataUnlocked()&&!!s&&String(s.uid||'')===ctx.uid&&
    String(personalWid())===ctx.wid&&String(activeWs()||'')===ctx.active&&signOutEpoch===ctx.epoch&&
    String(localStorage.getItem(OWNERKEY)||'')===ctx.seal&&workspaceSwitchGuardRaw()===ctx.switchSeal&&workspaceSwitchEpochRaw()===ctx.switchEpoch&&
    (typeof guard!=='function'||!!guard());}catch(_){return false;}
}
function personalReviewRead(){
  var a;try{a=JSON.parse(localStorage.getItem(PERSONAL_REVIEW)||'[]');}catch(_){throw syncIssue('sync_storage','personal_review_read','개인 자료 확인 목록을 읽지 못했습니다');}
  if(!Array.isArray(a))throw syncIssue('sync_storage','personal_review_read','개인 자료 확인 목록을 읽지 못했습니다');
  return a;
}
function personalReviewWrite(a){
  var raw=JSON.stringify(a);try{localStorage.setItem(PERSONAL_REVIEW,raw);}catch(_){throw syncIssue('sync_storage','personal_review_write','개인 자료 확인 목록을 저장하지 못했습니다');}
  if(localStorage.getItem(PERSONAL_REVIEW)!==raw)throw syncIssue('sync_storage','personal_review_write','개인 자료 확인 목록을 저장하지 못했습니다');
  try{window.dispatchEvent(new CustomEvent('ps-sync-state'));}catch(_){}
}
function personalReviewList(){
  try{var c=personalContext();if(!c)return [];return personalReviewRead().filter(function(x){
    return x&&x.uid===c.uid&&x.wid===c.wid&&PERSONAL[x.k];}).map(function(x){return Object.assign({},x,{viewActive:c.active,viewEpoch:c.switchEpoch,viewSeal:c.seal});});}catch(_){return [];}
}
function personalReviewSame(a,b){
  return !!a&&!!b&&a.uid===b.uid&&a.wid===b.wid&&a.k===b.k&&a.h===b.h&&a.c===b.c&&a.sh===b.sh;
}
function personalReviewGet(ctx,k){
  return personalReviewRead().filter(function(x){return x&&x.uid===ctx.uid&&x.wid===ctx.wid&&x.k===k;})[0]||null;
}
function personalReviewRecord(ctx,k,loc,row){
  if(!personalCurrent(ctx))throw syncIssue('sync_workspace_changed','personal_review_owner','개인 자료 확인 중 작업 공간이 바뀌었습니다');
  var entry={uid:ctx.uid,wid:ctx.wid,k:k,h:hash(loc),c:row.cupd,sh:hash(row.v),at:Date.now()},old=null;
  var a=personalReviewRead().filter(function(x){if(x&&x.uid===ctx.uid&&x.wid===ctx.wid&&x.k===k){old=x;return false;}return true;});
  if(personalReviewSame(old,entry))return old;
  a.push(entry);personalReviewWrite(a);return entry;
}
function personalReviewClear(ctx,k){
  if(!personalCurrent(ctx))throw syncIssue('sync_workspace_changed','personal_review_clear','개인 자료 확인 중 작업 공간이 바뀌었습니다');
  var a=personalReviewRead(),next=a.filter(function(x){return !(x&&x.uid===ctx.uid&&x.wid===ctx.wid&&x.k===k);});
  if(a.length!==next.length)personalReviewWrite(next);
}
function personalReviewChoose(k,keepMine,shown){
  var ctx=personalContext();if(!ctx||!shown||shown.uid!==ctx.uid||shown.wid!==ctx.wid||shown.k!==k)return Promise.resolve(false);
  if(shown.viewActive!==ctx.active||shown.viewEpoch!==ctx.switchEpoch||shown.viewSeal!==ctx.seal)return Promise.resolve(false);
  /* 화면을 연 뒤 새 입력/다른 계정/팀 전환이 생겼으면 이전 선택을 새 내용에 적용하지 않는다. */
  return currentValueForKey(k).then(function(loc){
    if(!personalCurrent(ctx)||hash(loc)!==shown.h)return false;
    var a=personalReviewRead(),chosen=false;
    a=a.map(function(x){if(personalReviewSame(x,shown)){chosen=true;return Object.assign({},x,{choice:keepMine?'local':'server'});}return x;});
    if(!chosen)return false;personalReviewWrite(a);return {pending:true};
  });
}
/* 이미 확인한 서버 변경도 보호한다. CAS만 붙여서 올리면 읽기 전에
   도착한 다른 기기의 변경을 덮기 때문에, 양쪽 변경은 반드시 명시 선택을 거친다. */
function personalPlan(ctx,k,loc,row,confirmed){
  ...
  var h=hash(loc),dirty=h!==((confirmed.h||{})[k]||''),changed=!!row&&row.cupd!==((confirmed.c||{})[k]||0);
  if(loc==null&&!row)return {action:'none'};
  if(row&&typeof row.v!=='string'&&(loc==null||changed||personalReviewGet(ctx,k)))
    throw syncIssue('sync_confirm_missing','personal_pull_missing','개인 자료의 최신 원문을 확인하지 못했습니다');
  if(row&&row.v===loc)return {action:'equal',raw:loc,cupd:row.cupd};
  if(loc==null)return {action:'pull',raw:row.v,cupd:row.cupd};
  if(!row)return {action:'push',raw:loc,missing:true};
  if(dirty&&changed&&hash(row.v)!==((confirmed.h||{})[k]||'')){
    var entry=personalReviewRecord(ctx,k,loc,row);
    if(entry.choice==='local')return {action:'push',raw:loc,cupd:row.cupd};
    if(entry.choice==='server')return {action:'pull',raw:row.v,cupd:row.cupd};
    return {action:'review'};
  }
  if(dirty)return {action:'push',raw:loc,cupd:row.cupd};
  if(changed)return {action:'pull',raw:row.v,cupd:row.cupd};
  return {action:'none'};
}
function syncPersonal(at,ownerGuard){
  var ctx=personalContext(),team=isTeamWs();
  function current(){return personalCurrent(ctx,ownerGuard);}
  ...
  requireCurrent('personal_start');
  var pwid=ctx.wid;
  if(!team&&ctx.active!==pwid)throw syncIssue('sync_workspace_changed','personal_workspace','개인 공간을 확인하지 못했습니다');
  var keys=KEYS.filter(function(k){ return PERSONAL[k]; });
  if(!keys.length) return Promise.resolve(0);
  var startMeta=meta(),confirmed=team?(startMeta.p||{}):startMeta;
  confirmed={h:Object.assign({},confirmed.h||{}),c:Object.assign({},confirmed.c||{})};
  return Promise.all(keys.map(function(k){return currentValueForKey(k).then(function(raw){return [k,raw];});})).then(function(pairs){
    ...
    function locOf(k){return vals[k]===undefined?null:vals[k];}
    return syncFetch('personal_meta',base0+'&k=in.('+keys.map(encodeURIComponent).join(',')+')&select=k,cupd',{headers:hj(at)}).then(function(r){
      ...
      if(!Array.isArray(metaRows))throw syncIssue('sync_confirm_missing','personal_meta_shape','개인 자료 목록을 확인하지 못했습니다');
      var need=[]; metaRows.forEach(function(r0){ var loc=locOf(r0.k); if(loc==null||r0.cupd!==(confirmed.c[r0.k]||0)||importApproved(r0.k,loc)||personalReviewGet(ctx,r0.k))need.push(r0.k); });
      ...
      return syncFetch('personal_pull',base0+'&k=in.('+need.map(encodeURIComponent).join(',')+')&select=k,v,cupd',{headers:hj(at)}).then(function(r){
        ...
        if(!Array.isArray(vrows))throw syncIssue('sync_confirm_missing','personal_pull_shape','개인 자료 원문을 확인하지 못했습니다');
        var vm={}; vrows.forEach(function(x){vm[x.k]=x;}); return metaRows.map(function(r0){ return vm[r0.k]||r0; }); });
    }).then(function(rows){
      ...
      try{keys.forEach(function(k){
        ...
        if(row&&row.v!==loc&&importApproved(k,loc)){
          ...
          if(personalRaw==null)throw syncIssue('sync_confirm_missing','personal_import_rebase','개인 경기 메모를 합치지 못했습니다');
          if(personalRaw!==loc){if(!personalWrite(k,personalRaw,loc))throw syncIssue('sync_local_changed','personal_import_write','새 개인 편집을 확인했습니다');applied++;loc=personalRaw;}
          plan={action:'push',raw:loc,cupd:row.cupd};
        }
        if(!plan)plan=personalPlan(ctx,k,loc,row,confirmed);
        ...
        if(plan.action==='push'){
          ...
          writes.push(outboxMarkForOwner(ctx.uid,pwid,k,hash(plan.raw),'personal-push'));return;
        }
        if(plan.action==='pull'){
          if(!personalWrite(k,plan.raw,loc))throw syncIssue('sync_local_changed','personal_pull_write','개인 자료를 받는 동안 새 편집을 확인했습니다');
          applied++;
        }
      ...
      return Promise.all(writes).then(function(){
        requireCurrent('personal_write_done');
        if(writeGuards.some(function(g){return g.stale;}))throw syncIssue('sync_local_changed','personal_write_stale','개인 자료에 새 편집이 있습니다');
        return Promise.all(pushRows.map(function(req){return currentValueForKey(req.k).then(function(raw){
          if(raw!==candidates[req.k])throw syncIssue('sync_local_changed','personal_push_stale','개인 자료에 새 편집이 있습니다');
        });}));
      }).then(function(){
        requireCurrent('personal_push');
        return kvPushRows(at,pushRows,function(sent){(sent||[]).forEach(function(req){accepted[req.k]={raw:req.v,cupd:req.cupd};});},'personal_push',current);
      }).then(function(){
        requireCurrent('personal_push_done');
        if(pushRows.some(function(req){return !accepted[req.k]||accepted[req.k].raw!==req.v;}))
          throw syncIssue('sync_confirm_missing','personal_push_confirm','개인 자료의 서버 저장을 확인하지 못했습니다');
        /* 다른 키의 최신 meta를 보존하고 이 회차의 확정 키만 반영한다. */
        var m=meta(),target=team?(m.p=m.p||{}):m;target.h=target.h||{};target.c=target.c||{};
...
function syncNow(reason){
  ...
  if(importSyncLock){
    if((reason==='team-section-import'||reason==='team-section-preflight'||reason==='history-restore-preflight'||reason==='history-restore')&&lockWid===importSyncWid)return syncNowCore(reason);
...
function syncNowCore(reason){
  ...
  return Promise.resolve(sharedReady).then(function(){return itemsWriteFlush();}).then(function(){return syncBasePrimeAll(wid,syncBaseMetaSnapshot);}).then(function(){return ensureToken();}).then(function(at){
    ...
    /* 1.619 — 권한 문서를 IndexedDB 에서 먼저 확보한다. 아래 푸시 판정이 동기라
      ...
      metaRows.forEach(function(r0){
        ...
        if(k===MATCH_KEY){
          /* 준비 표는 localStorage를 동기로 읽는다. 워크스페이스 확인표가 없거나
             ...
             준비 표가 있어도 서버 cupd가 바뀌었으면 반드시 v를 받아야 한다. 예전 코드의
             조기 return은 이 조건을 건너뛰어 undefined를 서버 원문처럼 쓰는 길을 만들었다. */
          var matchIdb=(idbVals[k]===undefined?null:idbVals[k]),matchMirror=null;
          try{matchMirror=localStorage.getItem(k);}catch(_){}
          if(!keyReady(k,wid)||matchMirror==null||matchIdb==null||matchMirror!==matchIdb||
             r0.cupd!==(m0.c[k]||0)||psCount(matchIdb)===0||importApproved(k,matchIdb))needV.push(k);
          return;
        }
        if(k===SCHEDULE_KEY){
        ...
        if(isIdpPrivateKey(k)&&s&&s.uid&&k!==IDP_PRIV_PREFIX+s.uid){
          var foreignLoc=null;try{foreignLoc=localStorage.getItem(k);}catch(_){}
          if(foreignLoc!=null&&hash(foreignLoc)!==(m0.h[k]||'')){needV.push(k);return;}
        }
        ...
        else if(ITEMS_ACTIVE&&isItemKey(k)&&itemsDeletedLive(k,loc))needV.push(k); /* 삭제 기록과 엇갈리는 live 원문은 자동 재업로드 전에 검토 */
      ...
      var srv={}; (rows||[]).forEach(function(r0){ srv[r0.k]=r0; });
      var m=meta(), pushRows=[], teamPushCandidates={},confirmedPushKeys={}, idpBaseNext={},idpPushCandidate={},idpPushConfirmed={},idpLocalStage={},idpCommitDone=false, applied=0, skippedBig=0, skippedKeys=[], heldKeys=[], dependencyDeferredKeys=[], now=Date.now(), writes=[], itemsApplied=0,itemsObservedChanged=false,matchReadyBlocked=false;
      ...
      function invalidateMatchReadyLocal(){
        ...
        /* 현재 회차의 m은 마지막에만 저장된다. 화면이 그 사이 열릴 수 있으므로
           이미 저장된 meta의 표도 즉시·exact로 닫는다. */
        matchReadyInvalidate();
      }
      function restoreMatchMeta(){
        if(matchMetaBefore.h===undefined)delete m.h[MATCH_KEY];else m.h[MATCH_KEY]=matchMetaBefore.h;
      ...
      function blockMatchReady(stage,error){
        ...
        restoreMatchMeta();
        if(stage)syncDiagnostic(stage,error||new Error('경기 자료 확인 대기'));
      }
      function expectMatchRaw(raw,resolved){
        if(raw!==null&&typeof raw!=='string'){
          blockMatchReady('match-expected-raw',new Error('경기 확인 원문이 없습니다'));
          return false;
        }
      ...
      function roundKvWrite(k,v,writes0,expectedLoc,writeGuard){
        ...
        if(k===MATCH_KEY){
          ...
          /* 거울 복구·순수 pull도 preload 후 사용자 편집과 겹칠 수 있다.
             경기 쓰기는 전부 expectedLoc CAS를 타게 해 느린 동기화가 새 편집을
             예전 preload raw로 되돌리지 않게 한다. */
      ...
      function pushHoldTracked(k,loc,srvVal,memN){
        ...
        if(held){
          ...
          if(k===MATCH_KEY)blockMatchReady();
      ...
      function queuePush(k,raw,confirmedRaw,expectedLoc,casCupd,casMissing){
        ...
        if(k===SCHEDULE_KEY){
          ...
          if(send!==expectedLoc){
            if(!roundKvWrite(k,send,writes,expectedLoc,scheduleGuard)){
        ...
        if(k===MATCH_KEY){
          invalidateMatchReadyLocal();matchPushPlanned=true;
          if(!expectMatchRaw(send,false))return null;
        }
        /* Every shared document is conditional on the row read in this round.
           Two coaches reading the same version must not both overwrite it with
           whole documents, even when they edited different matches or fields. */
      ...
      function restoreUnconfirmedTeamKey(k){
        var candidate=teamPushCandidates[k];if(!candidate||candidate.confirmed)return;
      ...
      function resolveTeamConflict(k,loc,row,forceReview){
        ...
        if(held.choice==='server'){
          if(roundKvWrite(k,row.v,writes,loc,k===SCHEDULE_KEY?scheduleGuard:null)){
            ...
            m.h[k]=hash(row.v);m.c[k]=row.cupd;nSet(m,k,row.v);syncBaseSet(k,row.v);
          }
          return;
        }
        if(heldKeys.indexOf(k)<0)heldKeys.push(k);
        if(k===MATCH_KEY)blockMatchReady('match-review-needed',new Error('두 기기의 경기 변경을 그대로 보관하고 있습니다'));
        if(k===SCHEDULE_KEY){scheduleDeferred=true;schedulePushHeld=true;}
      }
      function idpConflictChoice(k,loc,row,merged){
        if(!idpMergeNeedsReview(merged))return '';
        var held=holdConflictRecord(k,loc,row);
        if(held.choice==='local'||held.choice==='server')return held.choice;
        if(heldKeys.indexOf(k)<0)heldKeys.push(k);
        return 'pending';
      }
      var teamWs=isTeamWs();
      KEYS.forEach(function(k){
        ...
        if(k===MATCH_KEY&&loc!=null){
          var recoveryApproval=importApprovalGet(k,loc),recoverySpec=recoveryApproval&&recoveryApproval.s;
          if(recoverySpec&&recoverySpec.section==='match-recovery'){
            /* KEYS에서 삭제 기록은 MATCH 뒤에 온다. meta에서 이미 새 tombstone을 보았는데
               옛 local tombstone으로 MATCH를 먼저 올리면 명시 삭제가 잠깐이라도 부활한다. */
            var tombServerRow=srv[MATCH_DEL_KEY],tombServerCupd=tombServerRow?Math.max(0,Math.floor(+tombServerRow.cupd||0)):0;
            if(tombServerCupd!==Math.max(0,Math.floor(+recoverySpec.tombstoneCupd||0))){
              if(dependencyDeferredKeys.indexOf(MATCH_KEY)<0)dependencyDeferredKeys.push(MATCH_KEY);
              blockMatchReady('match-recovery-tombstone-moved',new Error('경기 삭제 기록이 바뀌어 복구를 다시 확인합니다'));
              return;
            }
          }
        }
        ...
        if(k===MATCH_KEY&&row&&matchFullRequested&&typeof row.v!=='string'){
          ...
          blockMatchReady('match-full-missing',new Error('경기 서버 원문을 확인하지 못했습니다'));
          return;
        }
        ...
        if(k===SCHEDULE_KEY&&row&&row.v!=null&&!scheduleWriteAllowed()){
          ...
          if(roundKvWrite(k,row.v,writes,loc,scheduleGuard)){
            applied++;scheduleAppliedPlanned++;m.h[k]=hash(row.v);m.c[k]=row.cupd;nSet(m,k,row.v);syncBaseSet(k,row.v);
          }
          importApprovalClear(k);
          return;
        }
        if(loc==null&&row){ if(roundKvWrite(k,row.v,writes,loc,k===SCHEDULE_KEY?scheduleGuard:null)){ applied++;if(k===SCHEDULE_KEY)scheduleAppliedPlanned++; m.h[k]=hash(row.v); m.c[k]=row.cupd; nSet(m,k,row.v); syncBaseSet(k,row.v); } return; }
        /* IDB에는 서버와 같은 경기 문서가 있지만 거울만 빠진 상태가 이번 사고의 직접 조건이다.
           "양쪽 그대로"로 끝내지 말고 같은 원문을 다시 써 exact mirror를 복구한다. */
        if(k===MATCH_KEY&&row&&row.v!=null&&loc===row.v){
          var mirrorNow=null;try{mirrorNow=localStorage.getItem(k);}catch(_){}
          if(mirrorNow!==loc){
            if(roundKvWrite(k,row.v,writes,loc,null)){applied++;m.h[k]=hash(row.v);m.c[k]=row.cupd;nSet(m,k,row.v);syncBaseSet(k,row.v);}
            return;
          }
          /* full pull에서 받은 서버 raw와 IDB·거울이 이미 같다. 이 raw만
             commit의 세 번째 비교 값으로 사용한다. */
          expectMatchRaw(row.v,true);
        }
        ...
        if(row&&row.v!==loc&&importApproved(k,loc)){
          ...
          if(approvedRaw!==loc){
            if(!roundKvWrite(k,approvedRaw,writes,loc,k===SCHEDULE_KEY?scheduleGuard:null)){importApprovalClear(k);return;}
            applied++;if(k===SCHEDULE_KEY)scheduleAppliedPlanned++;
          }
        ...
        if(loc!=null&&!row){
          /* 이력 경기 복구는 서버 행이 아직 없는 경우도 승인 raw를 그대로 INSERT하지
             않는다. 승인 뒤 생긴 삭제 묘비를 다시 투영한 뒤, 행 없음 CAS로만 만든다. */
          var missingImport=k===MATCH_KEY?importApprovalGet(k,loc):null;
          if(missingImport&&missingImport.s&&missingImport.s.section==='match-recovery'){
            var missingRebase=importApprovedRebase(k,loc,null),missingRaw=missingRebase&&missingRebase.raw;
            if(missingRaw==null){blockMatchReady('import-rebase-pending',new Error('경기 복구 삭제 기록 확인 대기'));return;}
            if(missingRaw!==loc){
              var missingGuard={stale:false};
              if(!roundKvWrite(k,missingRaw,writes,loc,missingGuard)){importApprovalClear(k);return;}
              applied++;
            }
            queuePush(k,missingRaw,null,missingRaw,null,true);return;
          }
          if(pushHoldTracked(k,loc,null,m.n[k])) return;
          if(k===MATCH_KEY){var mm0=null;try{mm0=localStorage.getItem(k);}catch(_){}if(mm0!==loc&&!roundKvWrite(k,loc,writes,loc,null))return;}
        ...
        if(dirty&&!srvChanged){
          ...
          if(k===MATCH_KEY){var mm1=null;try{mm1=localStorage.getItem(k);}catch(_){}if(mm1!==loc&&!roundKvWrite(k,loc,writes,loc,null))return;}
          queuePush(k,loc,row&&row.v!=null?row.v:null,loc);return; }
        if(!dirty&&srvChanged){ if(roundKvWrite(k,row.v,writes,loc,k===SCHEDULE_KEY?scheduleGuard:null)){ applied++;if(k===SCHEDULE_KEY)scheduleAppliedPlanned++; m.h[k]=hash(row.v); m.c[k]=row.cupd; nSet(m,k,row.v); syncBaseSet(k,row.v); } return; }
        if(dirty&&srvChanged){
          ...
          if(k===SCHEDULE_KEY){
            ...
            if(!trustedBase||hash(trustedBase)!==(m.h[k]||'')){
              ...
              resolveTeamConflict(k,loc,row);
              return;
            }
          }
          /* With no proven merge base, server-wins would erase the unsent body.
             COPY_OFF also disables rescue, so retain both and review only a real
             simultaneous edit. Stable-ID roster merging is deliberately absent. */
          if(!MERGE_KEYS[k]||k===MATCH_KEY){resolveTeamConflict(k,loc,row);return;}
          /* Schedules alone have a proven date-based merge rule. */
          if(MERGE_KEYS[k]){
            var mg=MERGE_LIST[k]
          ...
          resolveTeamConflict(k,loc,row);return;
        }
        /* 양쪽 다 그대로 — 병합 키는 base가 없으면 지금 상태를 base로 */
        if(MERGE_KEYS[k]&&loc!=null&&!syncBaseGet(k))syncBaseSet(k,loc);
      });
      /* full raw를 요청한 회차는 그 raw, 서버 부재, 또는 성공할 push 중
         하나로 결론나야 한다. 어느 분기도 그 결론을 만들지 못했다면
         로컬 두 사본이 우연히 같아도 ready를 내줄 수 없다. */
      if(matchFullRequested&&!matchReadyBlocked&&!matchResolutionPlanned){
      ...
      (function(){
        ...
        function queueMinePush(k,cloud,localRaw,row,before){
          ...
          if(cloud.length>MAXLEN){skippedBig++;if(skippedKeys.indexOf(k)<0)skippedKeys.push(k);return false;}
        ...
        Object.keys(set).forEach(function(k){
          ...
          /* Team-only image notes remain on this device. Apply the upload cap
             to the actual cloud document so they cannot block a new diary. */
          var sizeRaw=mine&&teamW&&loc!=null?pushVal(loc):loc;
          if(sizeRaw!=null&&sizeRaw.length>MAXLEN){ skippedBig++; skippedKeys.push(k); return; }
          ...
          if(mine){
            ...
            if(dirty&&srvChanged){
              ...
              if(mineChoice==='server')merged={local:keepMyImg(row.v,loc),cloud:pushVal(row.v)};
              if(typeof merged.local!=='string'||typeof merged.cloud!=='string')return;
            ...
            /* Foreign private IDP has no authoring path on this account. Restore its
               read-only cache from the server, including local photo references. The
               reference must be durable before replacing raw/confirming the cache hash;
               a later fire-and-forget strip would dirty the just-ACKed cache again. */
            if(row&&(loc==null||dirty||row.cupd!==(m.c[k]||0))){
              if(typeof row.v!=='string'){syncDiagnostic('idp-private-other-missing',new Error('다른 선수 private IDP 서버 원문이 없음'));return;}
              if(!idpRawMergeable(k,row.v)){syncDiagnostic('idp-private-other-shape',new Error('다른 선수 private IDP 서버 판본을 확인할 수 없음'));return;}
              var otherRaw=stripOtherImg(row.v);
              if(typeof otherRaw!=='string'){syncDiagnostic('idp-private-other-shape',new Error('다른 선수 private IDP 원문을 투영할 수 없음'));return;}
              writes.push(Promise.resolve().then(function(){
                requireRoundWorkspace('idp_other_projection');
                var img=window.PSImg,projected=JSON.parse(otherRaw),photo=projected.profile&&projected.profile.photo;
                if(!img||!img.isData||!img.isData(photo))return otherRaw;
                if(typeof img.ready!=='function'||typeof img.put!=='function'||typeof img.src!=='function')
                  throw syncIssue('sync_storage','idp_other_photo','선수 사진 저장소를 확인하지 못했습니다');
                return Promise.resolve(img.ready()).then(function(){
                  requireRoundWorkspace('idp_other_photo_ready');
                  var oldDoc=null;try{oldDoc=JSON.parse(loc);}catch(_){}
                  var oldRef=oldDoc&&oldDoc.profile&&oldDoc.profile.photo;
                  if(img.isRef&&img.isRef(oldRef)&&img.src(oldRef)===photo&&typeof img.verify==='function'){
                    return Promise.resolve(img.verify(oldRef,photo)).then(function(ok){
                      requireRoundWorkspace('idp_other_photo_verified');return ok?oldRef:img.put(photo);
                    });
                  }
                  return img.put(photo);
                }).then(function(ref){
                  requireRoundWorkspace('idp_other_photo_commit');
                  if(!img.isRef||!img.isRef(ref)||img.src(ref)!==photo)
                    throw syncIssue('sync_storage','idp_other_photo','선수 사진 저장 결과를 확인하지 못했습니다');
                  projected.profile.photo=ref;return JSON.stringify(projected);
                });
              }).then(function(cacheRaw){
                requireRoundWorkspace('idp_other_cache_commit');
                if(!roundKvWrite(k,cacheRaw,writes,loc,{skipIdpStrip:true}))
                  throw syncIssue('sync_local_changed','idp_other_cache_changed','선수 읽기 자료가 확인 중 바뀌었습니다');
                if(localStorage.getItem(k)!==cacheRaw)
                  throw syncIssue('sync_storage','idp_other_cache_verify','선수 읽기 자료 저장을 확인하지 못했습니다');
                applied++;m.h[k]=hash(cacheRaw);m.c[k]=row.cupd;
              }));
            }
          }
        });
      ...
      (function(){
        ...
        Object.keys(set2).forEach(function(k){
          ...
          if(dirty&&srvChanged){
            ...
            var pubChoice=idpConflictChoice(k,loc,row,pubMerged);if(pubChoice==='pending')return;
            if(pubChoice==='local')pubMerged={local:loc,cloud:loc};
            if(pubChoice==='server')pubMerged={local:row.v,cloud:row.v};
      ...
      (function(){
        ...
        itemsObservedChanged=itemsObserveServer(srv);
        ...
        var deleteReview={};itemsDeleteReviews().forEach(function(x){deleteReview[x.itemKey]=1;});
        keys.forEach(function(k){
          if(deleteReview[k]){if(heldKeys.indexOf(k)<0)heldKeys.push(k);return;}
          ...
          /* 올릴 때 내가 본 판본(cupd)을 붙인다(_casCupd → PATCH cupd=eq).
             CAS가 거부되면 itemsResolveConflicts가 자료 검토를 남기고 원문을 보존한다.
             새 행은 _casMissing(ignore-duplicates)로 다른 기기의 생성을 덮지 않는다. */
          if(loc!=null&&!row){
            if(!itemsCreateAllowed(k,loc))return; /* 기기에 남은 옛 행은 보존하되 현역으로 자동 업로드하지 않는다 */
            pushRows.push({workspace_id:wid,k:k,v:loc,cupd:now,_casMissing:true,_casSoft:true}); m.h[k]=lh; m.c[k]=now; return;
          }
          ...
          if(dirty&&srvChanged){
            /* Preserve both originals until the same exact review used by other
               shared data authorizes one player's complete document. */
            resolveTeamConflict(k,loc,row);
            if(heldKeys.indexOf(k)>=0)writes.push(outboxMarkForOwner(s.uid,wid,k,lh,'roster-conflict'));
          }
        });
      ...
      (function(){
        ...
        deniedKeys.forEach(function(k){cancelIdpCandidate(k);restoreUnconfirmedTeamKey(k);});
      ...
      function commitMatchReady(){
        /* 기존의 유효한 준비표를 routine defer 하나 때문에 닫지는 않는다. 다만 이번 회차가
           경기 키를 미뤘거나 거부·보류했다면 새 준비표를 절대 발급하지 않는다. */
        ...
        return waitMirror.then(function(){
          ...
          var idb=rec&&rec.value!=null?String(rec.value):null,mirror=null;
          try{mirror=localStorage.getItem(MATCH_KEY);}catch(e){throw e;}
          var marker=null;
          if(matchExpectedRaw===null&&idb===null&&mirror===null)marker={w:String(wid),present:false,h:''};
      ...
      return Promise.all(writes).then(function(){
        ...
        /* CAS가 막은 원문을 marker만 닫고 서버에는 계속 보내면, 바로 그 새 편집을
           옛 판으로 덮는다. 어떤 fail-closed 사유든 이 회차의 경기 push도 함께 제거한다. */
        if(matchReadyBlocked)pushRows=pushRows.filter(function(r){return r.k!==MATCH_KEY;});
        return syncBaseReady();
      }).then(function(){   /* 2.730 — IDB 쓰기가 끝난 뒤에 «행이 왔다» 신호 */
        ...
        return outboxMarkRows(wid,pushRows,m,s.uid,roundWorkspaceCurrent).then(function(){
          if(!scoutPushCurrent())deferScout('scout-local-changed');
          if(!schedulePushCurrent()){
            var hadMatch=pushRows.some(function(r){return r.k==='cs_team_matches_v1';});
            pushRows=pushRows.filter(function(r){return r.k!==SCHEDULE_KEY&&r.k!=='cs_team_matches_v1';});
          ...
          return kvPushRows(at,pushRows,function(ch){
            ...
            (ch||[]).forEach(function(sent){
              if(sent){confirmedPushKeys[sent.k]=1;m.c[sent.k]=sent.cupd;var candidate=teamPushCandidates[sent.k];if(candidate&&candidate.row===sent)candidate.confirmed=true;}
            ...
            Object.keys(teamPushCandidates).forEach(restoreUnconfirmedTeamKey);
        ...
        if(itemsApplied>0||itemsObservedChanged){try{localStorage.setItem('ps_items_rev',String(Date.now()));}catch(_){}} /* 첫 서버 확인도 확정 메타 뒤에 화면에 알린다 */
        ...
        var personalError=null;
        var doPersonal=Promise.resolve().then(function(){return syncPersonal(at,roundWorkspaceCurrent);}).then(function(n){applied+=n||0;})
          .catch(function(e){
            if(!roundWorkspaceCurrent())throw e;
            personalError=classifySyncError(e); /* 팀 저장은 완료됐지만 개인 실패 표시는 syncState에 남긴다. */
          });
        ...
        return doPersonal.then(function(){return holdConflictClearConfirmed(m,roundWorkspaceCurrent);}).then(function(){ return outboxAckSynced(wid,m,ackSkipped,lr&&lr.ackHash,roundWorkspaceCurrent); }).then(function(){
        busy=false; try{ clearTimeout(busyDog); }catch(_){}
        requireRoundWorkspace('round_ack_final');
        rosterSyncMark={uid:s.uid,wid:wid,seal:roundOwnerSeal,at:Date.now()};
        clearSyncRetry();
...
/* 2.733 — 전환 직후 일정이 먼저 바뀐 회차는 경기 문서를 의도적으로 미룬다.
   일반 자동 재시도(350ms)는 바로 뒤 location.reload()가 취소하므로,
   전환 체인 안에서 경기 키가 처리된 두 번째 회차까지 명시적으로 기다린다. */
function forceSyncPostSwitch(reason){
  return forceSync(reason).then(function(r){
    var deferred=Array.isArray(r&&r.deferred)?r.deferred:[];
    var stopped=r&&(r.error||r.offline||r.noauth||r.nows);
    if((deferred.indexOf(MATCH_KEY)>=0||!keyReady(MATCH_KEY,activeWs()))&&!stopped)
      return sleep(40).then(function(){return forceSync('postswitch-match');}).then(function(r2){
        if(!keyReady(MATCH_KEY,activeWs())){
          try{var e=new Error('전환 뒤 경기 자료 확인이 끝나지 않았습니다');e.psCode='match_not_ready';syncDiagnostic('postswitch-match-not-ready',e);}catch(_){}
          if(r2&&typeof r2==='object')r2.matchNotReady=1;
        }
        return r2;
      });
    return r;
...
function stashSnapshot(wid){
  ...
  var owner=String((getSess()||{}).uid||''),startEpoch=signOutEpoch,ownerSeal='';
  try{ownerSeal=String(localStorage.getItem(OWNERKEY)||'');}catch(_){}
  function assertStashOwner(){
    var now=String((getSess()||{}).uid||'');
    var seal='';try{seal=String(localStorage.getItem(OWNERKEY)||'');}catch(_){}
    if(!owner||now!==owner||String(activeWs()||'')!==String(wid)||signOutEpoch!==startEpoch||seal!==ownerSeal||!dataUnlocked())
      throw new Error('Account or workspace changed while creating the switch backup');
    return true;
  }
  assertStashOwner();
  var snap={};
  try{
    CONTENT.forEach(function(k){ var v=localStorage.getItem(k); if(v!=null)snap[k]=v; });
    for(var pi=0;pi<localStorage.length;pi++){var pk=localStorage.key(pi),pv;
      if(pk&&pk.indexOf('cs_idp_pub_v1_')===0&&(pv=localStorage.getItem(pk))!=null)snap[pk]=pv;}
  }catch(e){syncDiagnostic('workspace-stash-local-read',e);throw e;}
  assertStashOwner();
  ...
  var idb = (window.storage)
    ? idbSwitchKeys().then(function(keys){
        ...
        return Promise.all(keys.map(function(k){
          return window.storage.get(k).then(function(r){
            /* Public feedback is localStorage-backed. Preserve that live value if an
               obsolete IDB mirror of the same dynamic key also exists. */
            if(r&&r.value!=null&&(k.indexOf('cs_idp_pub_v1_')!==0||!Object.prototype.hasOwnProperty.call(snap,k)))snap[k]=r.value;
          }).catch(function(e){syncDiagnostic('workspace-stash-read',e);throw e;});
        }));
      }).then(function(){assertStashOwner();})
    : Promise.resolve();
  return idb.then(function(){
    assertStashOwner();
    var key='ps_ws_stash_'+wid, raw=JSON.stringify({at:Date.now(),ownerUid:owner,data:snap});
    if(window.storage)return window.storage.set(key,raw).then(function(){return window.storage.get(key);}).then(function(r){
      assertStashOwner();if(!r||r.value!==raw)throw new Error('Switch backup verification failed');
    });
    try{localStorage.setItem(key,raw);if(localStorage.getItem(key)!==raw)throw new Error('Switch backup verification failed');assertStashOwner();return Promise.resolve();}catch(e){return Promise.reject(e);}
...
function switchWorkspaceCore(wid, skipSave){
  ...
  return _pre.then(function(r){
    ...
    try{workspacePubLocalKeys().forEach(removeLocal);}catch(e){e.psSwitchWipe=true;throw e;}
    removeLocal(MKEY);
...
var _dnHydratePromise=null,_dnHydrateKey='',_dnHydrateDoneKey='',_dnHydrateRetryKey='',_dnHydrateRetryAt=0;
...
/* 진입할 때 이름 입력창을 열지 않는다. 이미 정한 이름만 서버에서 조용히
   복원하고, 이름이 없으면 계정 메뉴의 '표시 이름 설정'에서 직접 정한다.
   확인을 마친 계정·팀은 이번 진입 동안 반복 조회하지 않고 연결 실패만 재시도한다. */
...
function permsEditChanges(before,after,staffBefore,staffAfter,sections){
  var changes=[],ids=sections.map(function(s){return s.id;});
  if(staffBefore!==staffAfter)changes.push({field:'staffEdit',value:staffAfter?'edit':'view'});
  Object.keys(after).forEach(function(uid){
    var old=before[uid],now=after[uid];if(!old)return;
    var roleChanged=old.role!==now.role;
    if(roleChanged)changes.push({uid:uid,field:'role',value:now.role});
    if(old.playerId!==now.playerId)changes.push({uid:uid,field:'playerId',value:now.playerId,remove:!now.playerId});
    if(roleChanged||ids.some(function(id){return !!old.scopes[id]!==!!now.scopes[id];}))
      changes.push({uid:uid,field:'scopes',value:ids.filter(function(id){return now.role==='admin'||now.role==='executive'||now.scopes[id];}),preserveUnknown:!roleChanged,known:ids});
  });
  return changes;
}
function permsEditApply(opened,latest,changes){
  function object(v){return !!v&&typeof v==='object'&&!Array.isArray(v);}
  function own(v,k){return Object.prototype.hasOwnProperty.call(v,k);}
  function sameField(a,b,k){return own(a,k)===own(b,k)&&JSON.stringify(a[k])===JSON.stringify(b[k]);}
  if(!object(opened)||!object(latest)||(opened.members!=null&&!object(opened.members))||(latest.members!=null&&!object(latest.members)))throw new Error('권한 원문을 확인하지 못했습니다. 창을 다시 열어 주세요.');
  if(!changes.length)return null;
  var out=JSON.parse(JSON.stringify(latest)),initial=opened.members||{},current=latest.members||{};
  changes.forEach(function(change){
    var field=change.field,before=opened,live=latest,target=out;
    if(change.uid!=null){
      var uid=change.uid;
      if(['role','scopes','playerId'].indexOf(field)<0||own(initial,uid)!==own(current,uid)||
         (own(initial,uid)&&(!object(initial[uid])||!object(current[uid]))))throw new Error('팀원 설정이 바뀌었습니다. 창을 다시 열어 확인해 주세요.');
      before=own(initial,uid)?initial[uid]:{};live=own(current,uid)?current[uid]:{};
      if(!out.members)out.members={};
      if(!own(out.members,uid))Object.defineProperty(out.members,uid,{value:{},enumerable:true,writable:true,configurable:true});
      target=out.members[uid];
    }else if(field!=='staffEdit')throw new Error('변경할 권한 항목을 확인하지 못했습니다.');
    if(!sameField(before,live,field))throw new Error('다른 곳에서 같은 설정이 바뀌었습니다. 창을 다시 열어 확인해 주세요.');
    if(change.remove)delete target[field];
    else if(field==='scopes'&&change.preserveUnknown){
      var unknown=Array.isArray(live.scopes)?live.scopes.filter(function(id){return change.known.indexOf(id)<0;}):[];
      target[field]=unknown.concat(change.value);
    }else target[field]=JSON.parse(JSON.stringify(change.value));
  });
  return out;
}
function uiPerms(wa){
  ...
  membersOf(wa.id).then(function(rows){ mm.close();
    if(!current())return;
    rows=rows||[];
    var rawOpened=P.get(),cur=rawOpened||{v:1,defaultRole:'player',members:{}};
    var opened=JSON.parse(JSON.stringify(cur));
    if(!cur.members)cur.members={};
    var dark=document.body.classList.contains('fmdark');
    ...
    var state={},initialState={};   /* uid -> {role, scopes:{id:bool}} */
    ...
    editable.forEach(function(r){
      ...
      lockUI();
      initialState[uid]=JSON.parse(JSON.stringify(state[uid]));
      card.appendChild(d);
    });
    var saveError=document.createElement('div');saveError.setAttribute('role','alert');saveError.style.cssText='font-size:12px;color:#c24a46;line-height:1.5;margin-top:8px';card.appendChild(saveError);
    ...
    save.onclick=function(){
      /* Only user-changed fields are applied to the latest document. A link
         change must not materialize inherited scopes or rewrite other members. */
      var out;
      try{
        if(!current())throw new Error('계정이나 팀 권한이 바뀌었습니다. 창을 다시 열어 주세요.');
        var latest=P.get();if(latest==null&&rawOpened==null)latest=opened;
        out=permsEditApply(opened,latest,permsEditChanges(initialState,state,staffBefore,seOn,secs));
        if(out)P.set(out);
      }catch(e){saveError.textContent=e.message||'권한을 저장하지 못했습니다. 다시 확인해 주세요.';return;}
      ov.remove();
      if(!out)return;
...
window.__psOpenAccountLogout=openAccountLogout;
function consumeAccountAction(){
  try{
    if(window.top!==window)return;
    var q=new URLSearchParams(location.search);
    if(q.get('account')!=='logout')return;
    q.delete('account');
    history.replaceState(null,'',location.pathname+(q.toString()?'?'+q.toString():'')+location.hash);
    _accountLogoutHint=true;_popOpen=true;
  }catch(_){}
}
function renderUI(){
  try{
    ensureCSS();
    try{ setTimeout(ensureDisplayName,600); }catch(_){}   /* 기존 표시 이름을 조용히 복원한다 */
    ...
    if(_accountLogoutHint&&s){
      var logoutHint=document.createElement('div');logoutHint.className='ap-st';logoutHint.setAttribute('role','status');
      logoutHint.textContent=dataUnlocked()
        ?'아직 로그아웃되지 않았습니다. 아래 로그아웃을 누르면 저장 상태를 확인합니다.'
        :'로그아웃을 위해 자료를 확인하고 있습니다. 준비가 끝나면 아래 버튼을 눌러 주세요.';
      pop.appendChild(logoutHint);
    }
    ...
    if(!s){
      ...
      var logoutButton=pb('로그아웃','',function(){
        if(_accountLogoutHint&&!dataUnlocked()){_popOpen=true;renderUI();return;}
        _popOpen=false;signOut();
      });
      if(_accountLogoutHint&&!dataUnlocked())logoutButton.disabled=true;
...
function boot(){
  ...
  if(s&&!bootExternalSwitch){ loadWorkspaces().then(function(){
           ...
           }).then(function(){ return syncNow('boot'); }).then(function(r){cacheWaitEnd(false);return r;})
  ...
  window.addEventListener('storage',function(e){
    ...
    if(e.key===SKEY){
      if(sessionStorageOwnerChanged(e)){
        signOutEpoch++;
    ...
    eventMirror.catch(function(){});
    function eventOutboxMark(){
      var qk=(key==='cs_lib_rev'||key===LIBKEY||key===TOMBKEY)?'@library':key;
      return currentValueForKey(qk).then(function(raw){
        var fenced=eventFence();if(fenced.drop)return null;
        return eventQwid?outboxMarkForOwner(eventUid,eventQwid,qk,hash(raw),'edit'):null;
      });
    }
    /* outbox까지 같은 직렬 꼬리에 넣는다. 로그아웃·워크스페이스 전환 장벽은
       IDB만이 아니라 이 표시가 실제로 착지할 때까지 기다린다. */
    editOutboxCommit=editOutboxCommit.catch(function(err){syncDiagnostic('outbox-edit-tail-recover',err);}).then(function(){return eventMirror;}).then(function(commit){
      if(commit&&commit.drop)return null;
      var fenced=eventFence();if(fenced.drop)return null;
      try{return eventOutboxMark();}catch(err){return Promise.reject(err);}
    });
...
/* 2.733 — 경기 이력/전환 백업은 현재 문서를 통째로 과거 판으로 바꾸지 않는다.
   안정 ID(또는 양쪽에서 유일한 sourceId)로만 같은 경기를 찾고, 현재의 값은 유지한 채
   비어 있는 필드를 과거 판으로 채운다. 연결 근거가 없는 과거 경기는 판단해 버리지 않고 추가한다. */
function recoveryClone(v){try{return JSON.parse(JSON.stringify(v));}catch(_){return v;}}
function recoveryHasValue(v){
  if(v==null)return false;
  if(typeof v==='string')return !!v.trim();
  if(Array.isArray(v))return v.some(recoveryHasValue);
  if(typeof v==='object')return Object.keys(v).some(function(k){return recoveryHasValue(v[k]);});
  return true;
}
function recoverySig(v){try{return JSON.stringify(v);}catch(_){return String(v);}}
function recoveryArrayKey(v){
  if(!v||typeof v!=='object')return '';
  if(Array.isArray(v)){
    var head=v[0];return (typeof head==='string'||typeof head==='number')?'tuple:'+String(head):'';
  }
  var fields=['id','pid','sourceId','num'];
  for(var i=0;i<fields.length;i++)if(v[fields[i]]!=null&&String(v[fields[i]]))return fields[i]+':'+String(v[fields[i]]);
  return '';
}
function recoveryDeletedIds(raw){
  if(raw==null)return {};
  var list=null;try{list=JSON.parse(String(raw));}catch(_){throw new Error('삭제한 경기 기록을 읽을 수 없어 복구를 중단했습니다');}
  if(!Array.isArray(list))throw new Error('삭제한 경기 기록의 형식이 올바르지 않아 복구를 중단했습니다');
  var out={};list.forEach(function(id){if(id!=null&&String(id))out[String(id)]=1;});return out;
}
/* 일정 동기화가 경기 본문보다 먼저 도착했을 때 scout.matchBlank()로 만든 껍데기만
   과거 경기의 신원까지 되찾을 수 있다. 단순히 "내용 점수 0"으로 판정하면 새 필드나
   사용자가 0/false로 고른 값을 껍데기로 오인한다 — 생성기가 넣는 키·값만 허용하고,
   모르는 키는 실제로 빈 값일 때만 통과시키는 fail-closed 판정이다. */
function recoveryIsGeneratedShell(m){
  if(!m||typeof m!=='object'||Array.isArray(m))return false;
  var sid=String(m.sourceId||'');
  if(!/^schedx?:/.test(sid))return false;
  if(typeof m.createdAt==='number'&&isFinite(m.createdAt)&&typeof m.updatedAt==='number'&&isFinite(m.updatedAt)&&m.updatedAt>m.createdAt)return false;
  var emptyText={venue:1,homeAway:1,captainId:1,attack:1,defense:1,attackTransition:1,defenseTransition:1,
    briefing:1,scoreUs:1,scoreThem:1,resultSummary:1,reviewGood:1,reviewImprove:1,trainingAction:1,idpAction:1};
  var emptyList={squadIds:1,startingIds:1,fifaPlans:1,boardLinks:1};
  return Object.keys(m).every(function(k){
    var v=m[k];
    if(k==='id')return typeof v==='string'&&!!v;
    if(k==='sourceId')return String(v)===sid;
    if(k==='competition')return v===''||v==='팀 일정';
    if(k==='opponent'||k==='time'||k==='formation')return typeof v==='string';
    if(k==='date')return typeof v==='string'&&/^\d{4}-\d{2}-\d{2}$/.test(v);
    if(emptyText[k])return v==null||String(v).trim()==='';
    if(emptyList[k])return v==null||(Array.isArray(v)&&v.length===0);
    if(k==='prepPublished'||k==='reviewPublished')return v==null||v===false;
    if(k==='execution')return v==null||+v===0;
    if(k==='fifaMode')return v==null||v==='simple';
    if(k==='kind')return v==null||v==='official'||v==='friendly';
    if(k==='createdAt'||k==='updatedAt')return v==null||(typeof v==='number'&&isFinite(v)&&v>=0);
    return !recoveryHasValue(v);
  });
}
function recoveryRestoreGeneratedShell(shell,hist){
  /* 현재 일정 껍데기를 바탕으로 빈 본문만 채운다. formation·kind·fifaMode 같은
     현재 메타는 유지하고, 과거 id/createdAt만 참조 재연결을 위해 명시적으로 채택한다. */
  var restored=recoveryClone(shell)||{};
  recoveryFillBlank(restored,hist||{});
  if(hist&&hist.id!=null&&String(hist.id)!=='')restored.id=recoveryClone(hist.id);
  if(hist&&hist.createdAt!=null)restored.createdAt=recoveryClone(hist.createdAt);
  /* 게시 여부는 내용 복구와 다른 접근제어 결정이다. 껍데기의 비공개 상태를 유지한다. */
  restored.prepPublished=false;
  restored.reviewPublished=false;
  return restored;
}
function recoveryFillBlank(cur,old){
  if(!recoveryHasValue(cur))return recoveryHasValue(old)?recoveryClone(old):cur;
  if(Array.isArray(cur)&&Array.isArray(old)){
    if(!cur.length)return recoveryClone(old);
    var ci={},cc={},oc={},seen={};
    cur.forEach(function(x){var id=recoveryArrayKey(x);if(id){cc[id]=(cc[id]||0)+1;ci[id]=x;}seen[recoverySig(x)]=1;});
    old.forEach(function(x){var id=recoveryArrayKey(x);if(id)oc[id]=(oc[id]||0)+1;});
    old.forEach(function(x){
      var id=recoveryArrayKey(x),sig=recoverySig(x);
      if(id&&cc[id]===1&&oc[id]===1){
        /* [phaseKey,label] 같은 튜플은 첫 값이 정체성이다. 현재 라벨은 유지하고
           비어 있는 칸만 채운다. 일반 배열 병합으로 보내면 라벨 둘이 한 튜플에 붙는다. */
        if(id.indexOf('tuple:')===0&&Array.isArray(ci[id])&&Array.isArray(x)){
          for(var j=0;j<x.length;j++){
            if(!recoveryHasValue(ci[id][j]))ci[id][j]=recoveryClone(x[j]);
            else if(ci[id][j]&&x[j]&&typeof ci[id][j]==='object'&&typeof x[j]==='object')ci[id][j]=recoveryFillBlank(ci[id][j],x[j]);
          }
        }else recoveryFillBlank(ci[id],x);
      }else if(!seen[sig]){
        /* 안정 ID가 없는 프레임·기간·목록도 버리지 않는다. 현재 배열 뒤에 과거 원소를
           별도 항목으로 남기므로 중복 가능성보다 복구 불능을 피하는 쪽을 택한다. */
        cur.push(recoveryClone(x));seen[sig]=1;
      }
    });
    return cur;
  }
  if(cur&&old&&typeof cur==='object'&&typeof old==='object'&&!Array.isArray(cur)&&!Array.isArray(old)){
    Object.keys(old).forEach(function(k){
      /* execution의 0은 UI에서 '미평가' sentinel(입력값은 1~5)이다. 과거에 실제
         평가가 있으면 복구한다. 게시 false는 접근제어 결정이므로 true로 자동 승격하지 않는다. */
      if(k==='prepPublished'||k==='reviewPublished')cur[k]=cur[k]===true;
      else if(k==='execution'&&+cur[k]===0&&+old[k]>=1&&+old[k]<=5)cur[k]=+old[k];
      else if(!Object.prototype.hasOwnProperty.call(cur,k)||!recoveryHasValue(cur[k]))cur[k]=recoveryClone(old[k]);
      else cur[k]=recoveryFillBlank(cur[k],old[k]);
    });
  }
  return cur;
}
function matchRecoveryMergeRaw(currentRaw,backupRaw,deletedRaw,stat){
  var cur=null,old=null;
  var currentMissing=currentRaw==null;
  try{cur=JSON.parse(currentMissing?'null':String(currentRaw));}
  catch(_){throw new Error('현재 경기 원문이 손상되어 복구를 중단했습니다');}
  try{old=JSON.parse(backupRaw||'null');}catch(_){throw new Error('복구할 경기 판을 읽을 수 없습니다');}
  if(!old||!Array.isArray(old.matches))throw new Error('복구할 경기 판의 형식이 올바르지 않습니다');
  if(!currentMissing&&(!cur||!Array.isArray(cur.matches)))throw new Error('현재 경기 판의 형식이 올바르지 않아 복구를 중단했습니다');
  if(currentMissing)cur={version:old.version||1,matches:[]};
  cur=recoveryClone(cur);old=recoveryClone(old);
  var deleted=recoveryDeletedIds(deletedRaw);stat=stat&&typeof stat==='object'?stat:{};stat.skippedDeleted=0;
  var curId={},oldId={},curSid={},oldSid={};
  function count(list,field,out){list.forEach(function(m){var v=m&&m[field];if(v!=null&&String(v)){v=String(v);out[v]=(out[v]||0)+1;}});}
  count(cur.matches,'id',curId);count(old.matches,'id',oldId);count(cur.matches,'sourceId',curSid);count(old.matches,'sourceId',oldSid);
  if(Object.keys(curId).some(function(id){return curId[id]>1;}))throw new Error('현재 경기 판에 같은 ID가 두 개 있어 복구를 중단했습니다');
  if(Object.keys(oldId).some(function(id){return oldId[id]>1;}))throw new Error('복구할 경기 판에 같은 ID가 두 개 있어 복구를 중단했습니다');
  old.matches.forEach(function(hist){
    if(!hist||typeof hist!=='object')return;
    var target=null,id=hist.id==null?'':String(hist.id),sid=hist.sourceId==null?'':String(hist.sourceId),byId=null,bySid=null;
    /* 명시 삭제 묘비가 복구보다 우선한다. 현재에 같은 id가 남아 있어도 빈 값을
       채우지 않고, 과거 레코드 append나 shell의 old-id 채택도 하지 않는다. */
    if(id&&deleted[id]){stat.skippedDeleted++;return;}
    if(id&&curId[id]===1&&oldId[id]===1)byId=cur.matches.find(function(m){return m&&String(m.id)===id;})||null;
    if(sid&&curSid[sid]===1&&oldSid[sid]===1)bySid=cur.matches.find(function(m){return m&&String(m.sourceId)===sid;})||null;
    /* sourceId가 유일하고 현재 쪽이 입증된 생성 껍데기면, 새 임시 id 대신 과거의
       안정 id를 채택해 담당·개인 메시지 같은 match-id 참조도 다시 이어지게 한다.
       다만 그 old id를 다른 현재 경기가 이미 쓰면 어느 경기인지 자동 판단하지 않는다. */
    if(bySid&&recoveryIsGeneratedShell(bySid)){
      /* 과거 id를 다른 현재 경기가 이미 쓴다면 exact id가 sourceId보다 강한 근거다.
         과거 레코드를 그대로 append하면 중복 id가 생겨 selector·개인 참조가 모호해지므로,
         exact-id 경기의 빈 값만 복구하고 일정 껍데기는 건드리지 않는다. */
      if(byId&&byId!==bySid){recoveryFillBlank(byId,hist);return;}
      var ix=cur.matches.indexOf(bySid);
      if(ix>=0){cur.matches[ix]=recoveryRestoreGeneratedShell(bySid,hist);return;}
    }
    /* 일반 경기의 sourceId 일치만으로는 같은 본문이라고 단정하지 않는다. 특히 id가
       달라진 과거 레코드를 여기에 흡수하면 그 id를 가리키는 담당·개인 참조가 고아가 된다.
       sourceId 병합은 위의 입증된 생성 껍데기에만 허용하고, 나머지는 exact id만 병합한다. */
    target=byId;
    if(target)recoveryFillBlank(target,hist);
    /* 알려진 필드 allow-list로 '내용 없음'을 판단하면 새 필드나 상대 보드 프레임을
       통째로 놓친다. 연결 근거가 없는 과거 경기는 빈 껍데기여도 그대로 보존한다. */
    else cur.matches.push(recoveryClone(hist));
  });
  Object.keys(old).forEach(function(k){if(k!=='matches'){
    if(!Object.prototype.hasOwnProperty.call(cur,k)||!recoveryHasValue(cur[k]))cur[k]=recoveryClone(old[k]);
    else cur[k]=recoveryFillBlank(cur[k],old[k]);
  }});
  return JSON.stringify(cur);
}

/* 경기 이력 복구는 일반 history overwrite와 다르다. 현재 서버 판을 먼저 확정하고,
   복구분을 그 판 위에 투영한 원문만 CAS로 올린 뒤 서버 되읽기까지 맞아야 성공이다. */
function historyMatchApplyExact(backupRaw,restoreWid,assertRestoreWid){
  var locked=false;
  function tombstoneState(){
    assertRestoreWid();
    var raw=null;try{raw=localStorage.getItem(MATCH_DEL_KEY);}catch(e){throw new Error('삭제한 경기 기록을 읽지 못했습니다');}
    var ids=recoveryDeletedIds(raw);
    return {raw:raw,ids:Object.keys(ids)};
  }
  function readMatchExact(){
    assertRestoreWid();
    if(!window.storage||!window.storage.get)return Promise.reject(new Error('경기 안전 저장소를 열 수 없습니다'));
    var ready=window.PSStorage&&PSStorage.sharedReady?PSStorage.sharedReady(MATCH_KEY):true;
    return Promise.resolve(ready).then(function(){assertRestoreWid();return window.storage.get(MATCH_KEY);}).then(function(rec){
      assertRestoreWid();
      var idb=rec&&rec.value!=null?rec.value:null,mirror=null;
      if(idb!=null&&typeof idb!=='string')throw new Error('현재 경기 원문 형식이 올바르지 않습니다');
      try{mirror=localStorage.getItem(MATCH_KEY);}catch(e){throw new Error('경기 화면 원문을 읽지 못했습니다');}
      if(idb!==mirror)throw new Error('경기 안전 저장소와 화면 원문이 달라 복구를 중단했습니다');
      return idb;
    });
  }
  function validateCurrentBeforePreflight(tombRaw){
    assertRestoreWid();
    var mirror=null;try{mirror=localStorage.getItem(MATCH_KEY);}catch(e){throw new Error('경기 화면 원문을 읽지 못했습니다');}
    /* storage.get은 IDB가 비면 localStorage를 IDB로 옮긴다. 손상 거울은 그 호출보다
       먼저 거부해, 복구 실패가 손상 원문을 새 저장소에까지 복제하지 않게 한다. */
    if(mirror!=null)matchRecoveryMergeRaw(mirror,backupRaw,tombRaw,{});
    if(!window.storage||!window.storage.get)return Promise.reject(new Error('경기 안전 저장소를 열 수 없습니다'));
    var ready=window.PSStorage&&PSStorage.sharedReady?PSStorage.sharedReady(MATCH_KEY):true;
    return Promise.resolve(ready).then(function(){return window.storage.get(MATCH_KEY);}).then(function(rec){
      assertRestoreWid();
      var idb=rec&&rec.value!=null?rec.value:null;
      if(idb!=null&&typeof idb!=='string')throw new Error('현재 경기 원문 형식이 올바르지 않습니다');
      if(idb!=null)matchRecoveryMergeRaw(idb,backupRaw,tombRaw,{});
      if(mirror!=null&&idb!==mirror)throw new Error('경기 안전 저장소와 화면 원문이 달라 복구를 중단했습니다');
      return idb!=null?idb:mirror;
    });
  }
  function preflight(attempt){
    assertRestoreWid();
    return Promise.resolve(syncNow('history-restore-preflight')).then(function(res){
      assertRestoreWid();
      var retryable=!!(res&&(res.serverRejected||res.scheduleDeferred||res.skip||
        (Array.isArray(res.deferred)&&res.deferred.indexOf(MATCH_KEY)>=0)));
      var stopped=!res||res.offline||res.noauth||res.nows||res.locked||res.importLocked||res.error;
      if(stopped&&!retryable)return {ok:false};
      return readMatchExact().then(function(raw){
        if(keyReady(MATCH_KEY,restoreWid))return {ok:true,raw:raw};
        if(attempt<3)return sleep(180).then(function(){return preflight(attempt+1);});
        return {ok:false};
      });
    });
  }
  function approvalState(raw){
    var txt=null;try{txt=localStorage.getItem(importApprovalKey(MATCH_KEY));}catch(_){return 'invalid';}
    if(txt==null)return 'cleared';
    try{var rec=JSON.parse(txt);return rec&&rec.h===hash(raw)?'pending':'invalid';}catch(_){return 'invalid';}
  }
  function verifyServer(raw,spec){
    var written={};written[MATCH_KEY]=raw;
    return importVerifyExact(written,restoreWid,spec).then(function(v){
      if(!v||!v.ok||!keyReady(MATCH_KEY,restoreWid))return false;
      return readMatchExact().then(function(current){return current===raw&&keyReady(MATCH_KEY,restoreWid);});
    });
  }
  function finishRecoveryRollback(baseRaw){
    assertRestoreWid();matchReadyInvalidate();
    var m=meta();m.h[MATCH_KEY]=hash(baseRaw);m.c[MATCH_KEY]=0;nSet(m,MATCH_KEY,baseRaw);
    m.r=m.r||{};delete m.r[MATCH_KEY];setMeta(m);syncBaseSet(MATCH_KEY,baseRaw);
    /* 후보를 정확히 원래 판으로 되돌린 뒤에만 일반 push 승인까지 치운다.
       다음 명시 복구는 c=0/ready=false에서 최신 서버를 먼저 다시 받는다. */
    importApprovalClear(MATCH_KEY);holdClear(MATCH_KEY);return true;
  }
  function rollbackRecoveryCandidate(state,current){
    assertRestoreWid();
    if(current!==state.raw)return Promise.resolve(false);
    if(state.baseRaw!=null){
      var writes=[],guard={stale:false};
      if(!kvWrite(MATCH_KEY,state.baseRaw,writes,current,guard))return Promise.resolve(false);
      return Promise.all(writes).then(function(){
        assertRestoreWid();if(guard.stale)return false;
        return readMatchExact().then(function(raw){return raw===state.baseRaw?finishRecoveryRollback(state.baseRaw):false;});
      });
    }
    /* 프리플라이트에서 서버 행이 없었던 경우도 후보를 남겨 두면 다음 sync가 INSERT할
       수 있다. IDB와 거울이 아직 exact 후보일 때만 둘 다 지운다. 사이 편집은 복원한다. */
    if(!window.storage||!window.storage.del||!window.storage.set)return Promise.resolve(false);
    var mirror=null;try{mirror=localStorage.getItem(MATCH_KEY);}catch(_){return Promise.resolve(false);}
    if(mirror!==current)return Promise.resolve(false);
    matchReadyInvalidate();
    return Promise.resolve(window.storage.del(MATCH_KEY)).then(function(){
      assertRestoreWid();var after=null;try{after=localStorage.getItem(MATCH_KEY);}catch(_){after='__read_error__';}
      if(after!==current){
        /* 삭제 대기 중 새 편집이 왔다면 방금 지운 IDB를 그 새 거울로 되돌리고 후보는 건드리지 않는다. */
        return after!=null&&after!=='__read_error__'?Promise.resolve(window.storage.set(MATCH_KEY,after)).then(function(){return false;}):false;
      }
      try{localStorage.removeItem(MATCH_KEY);if(localStorage.getItem(MATCH_KEY)!==null)return false;}catch(_){return false;}
      return Promise.resolve(window.storage.get(MATCH_KEY)).then(function(rec){
        var raw=rec&&rec.value!=null?rec.value:null;return raw==null?finishRecoveryRollback(null):false;
      });
    });
  }
  function syncRecovery(state){
    assertRestoreWid();tombstoneState(); /* 손상/빈 문자열 묘비면 push 전에 fail closed */
    return readMatchExact().then(function(before){
      var beforeApproval=approvalState(before);
      if(beforeApproval==='invalid')return {done:true,value:false};
      if(beforeApproval==='cleared')return verifyServer(before,state.spec).then(function(ok){return {done:true,value:ok};});
      return Promise.resolve(syncNow('history-restore')).then(function(res){return {done:false,res:res};});
    }).then(function(step){
      if(step.done)return step.value;
      assertRestoreWid();
      return readMatchExact().then(function(current){
        var a=approvalState(current);
        /* 승인 토큰은 서버가 exact raw를 확인한 경로에서만 지워진다. 지워졌다면
           마지막으로 서버를 직접 되읽어 같은 원문인지 확인한다. */
        if(a==='cleared')return verifyServer(current,state.spec);
        if(a==='invalid')return false;
        /* CAS 충돌·후순위 tombstone 변경·네트워크 미확인 모두 자동 재병합하지 않는다.
           exact 후보일 때만 프리플라이트 base로 되돌려 다음 명시 시도가 최신 서버부터
           다시 읽게 한다. 이미 다른 편집이면 rollback도 하지 않고 fail closed다. */
        return rollbackRecoveryCandidate(state,current).then(function(){return false;});
      });
    });
  }
  function land(baseRaw){
    var tomb=tombstoneState(),stat={},raw=matchRecoveryMergeRaw(baseRaw,backupRaw,tomb.raw,stat);
    var m0=meta(),spec=importSpec({section:'match-recovery',tombstoneIds:tomb.ids,
      baseMissing:baseRaw==null,baseHash:hash(baseRaw),tombstoneHash:hash(tomb.raw),tombstoneCupd:m0.c&&m0.c[MATCH_DEL_KEY]});
    if(stat.skippedDeleted)try{syncDiagnostic('hist-restore-deleted-skip',new Error(String(stat.skippedDeleted)+' matches'));}catch(_){}
    /* 합칠 것이 없어도 preflight 시점 이후 서버가 움직였을 수 있으므로 exact 되읽기는 한다. */
    if(raw===baseRaw)return verifyServer(raw,spec);
    /* 로컬 착지보다 먼저 원문 해시 승인 토큰을 둔다. 이후 어느 단계가 실패해도
       이 raw가 일반 무조건 push 경로로 빠지지 않고 최신 서버 위 CAS rebase만 탄다. */
    if(!importApprovalPut(MATCH_KEY,raw,spec))return false;
    var writes=[],guard={stale:false};
    if(!kvWrite(MATCH_KEY,raw,writes,baseRaw,guard))return false;
    return Promise.all(writes).then(function(){
      assertRestoreWid();if(guard.stale)return false;
      return readMatchExact();
    }).then(function(current){
      if(current===false||current!==raw)return false;
      return importApproveExact(MATCH_KEY,raw,spec);
    }).then(function(approved){
      if(approved!==true)return false;
      return syncRecovery({raw:raw,baseRaw:baseRaw,spec:spec});
    });
  }
  return Promise.resolve().then(function(){assertRestoreWid();return syncImportBegin(restoreWid);}).then(function(ok){
    if(!ok)return false;locked=true;
    return flushWorkspaceFrames().then(function(){
      assertRestoreWid();
      /* 손상 current/backup/묘비를 먼저 검증한다. preflight가 손상 current를
         서버에 올린 뒤에야 오류를 찾는 순서를 만들지 않는다. */
      var tomb=tombstoneState();
      return validateCurrentBeforePreflight(tomb.raw).then(function(){return preflight(0);});
    }).then(function(base){if(!base||!base.ok)return false;return land(base.raw);});
  }).then(function(ok){if(locked)syncImportEnd(restoreWid);return ok===true;},function(e){
    if(locked)syncImportEnd(restoreWid);throw e;
  });
}
...
window.PSSync={signIn:signIn,signOut:signOut,syncNow:syncNow,session:getSess,dataUnlocked:dataUnlocked,keys:KEYS,state:syncState,   /* 2.625 상태 한 줄 */
  ...
  history:function(key,limit){
    var wid=activeWs(),wsName=(activeWsObj()||{}).name||'';
    function assertHistoryWid(){
      if(!wid||String(activeWs()||'')!==String(wid)||!dataUnlocked()){
        var e=new Error('팀 기록을 확인하는 동안 워크스페이스가 바뀌었습니다');e.psWorkspaceChanged=true;throw e;
      }
    }
    ...
    return namesRefresh(wid).catch(function(){}).then(function(){
      ...
      assertHistoryWid();
      return {installed:true,wid:wid,ws:wsName,rows:rows||[],names:nameMap()};
    }).catch(function(e){
  ...
  historyGet:function(id,expectedWid){
    var wid=String(expectedWid||activeWs()||'');
    function assertHistoryWid(){
      if(!wid||String(activeWs()||'')!==wid||!dataUnlocked()){
        var e=new Error('팀 기록을 불러오는 동안 워크스페이스가 바뀌었습니다');e.psWorkspaceChanged=true;throw e;
      }
    }
    return Promise.resolve().then(function(){assertHistoryWid();
      return rpc('ps_kv_history_get',{p_wid:wid,p_id:id});
    }).then(function(rows){assertHistoryWid();return (rows&&rows[0])||null;});
  },
  ...
  historyApply:function(k,v,opts){
    ...
    if(k===MATCH_KEY){
      return historyMatchApplyExact(String(v),restoreWid,assertRestoreWid).catch(function(e){
    ...
    return currentValueForKey(k).then(function(expected){
      ...
      return Promise.all(writes).then(function(){assertRestoreWid();
        ...
        var verify = idbBacked(k)
          ? window.storage.get(k).then(function(r){ return r?r.value:null; })
          : Promise.resolve((function(){ try{ return localStorage.getItem(k); }catch(_){ return null; } })());
        return verify.then(function(cur){
          assertRestoreWid();
          var mirror=null;try{mirror=localStorage.getItem(k);}catch(_){}
    ...
    }).catch(function(e){syncDiagnostic('hist-restore',e);if(e&&e.psWorkspaceChanged)throw e;return false;});
  },
  histLabel:keyLabel,
