...
function harness(options={}){
 ...
 const c={console,JSON,Math,Date,Object,Array,Number,String,Promise,Error,URLSearchParams,encodeURIComponent,location:{origin:'https://private-board.invalid',search:''},
  ...
  localStorage:{getItem(k){reads.push(['ls',k]);return ls.get(k)??null;},setItem(k,v){if(options.quota)throw Error('synthetic quota');writes.push(['ls',k,String(v)]);ls.set(k,String(v));},removeItem(k){throw Error('unexpected remove:'+k);}},
...
test('offline first edit resumes after reload only when the server confirms no personal board exists',async()=>{
 const h=harness();h.get(async()=>{throw Error('synthetic offline');});await h.boot();h.c.state=snap(112);await h.c._boardLiveWriteNow(false);assert.equal(h.record().pending,true);assert.equal(h.record().base,null);await ticks();
...
test('online retry confirms an empty server without reloading or replacing the visible draft',async()=>{
 const h=harness();h.get(async()=>{throw Error('synthetic offline');});await h.boot();h.c.state=snap(120);await h.c._boardLiveWriteNow(false);assert.equal(h.record().base,null);h.get(null);const before=copy(h.c.state);assert.equal(await h.c.boardPrivateRetry(),true);assert.deepEqual(copy(h.c.state),before);assert.equal(h.record().pending,false);assert.equal(h.saves.length,1);
});
