...
try{
 ...
 for(const width of (baseline?[1280,820]:[1280,820,390])){
  ...
  try{
   ...
   assert.equal(await f.getByRole('button',{name:'이름 변경',exact:true}).count(),0);await f.getByLabel('페이지 이름 수정',{exact:true}).click();await f.getByLabel(/페이지 이름/).fill('빌드업');await f.getByRole('button',{name:'저장',exact:true}).last().click();assert.equal(await f.locator('[data-ph="atk"]').innerText(),'빌드업');
