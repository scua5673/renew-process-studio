...
const out=process.env.PS_TEST_OUTPUT||path.join(root,'test-results/mobile-oauth-recovery',engine);
...
async function geometry(button){
  return button.evaluate(el=>{
    ...
    return {text:el.textContent.trim(),x:r.x,y:r.y,width:r.width,height:r.height,scale:v?.scale||1,
      visibleCenter:!!hit&&(hit===el||el.contains(hit)),hit:hit?.outerHTML.slice(0,220),logicalViewport:innerWidth,physicalWidth:r.width*(v?.scale||1),physicalHeight:r.height*(v?.scale||1)};
  });
}
...
try{
  ...
  for(const spec of [{width:393,height:852,device:'phone'},{width:507,height:900,device:'phone'},{width:820,height:1180,device:'ipad'}].filter(s=>process.env.PS_TEST_WIDTHS?process.env.PS_TEST_WIDTHS.split(',').includes(String(s.width)):!(engine==='webkit'&&s.device==='ipad'))){
    ...
    await context.addInitScript(({base})=>{
      if(location.origin!==base||window!==window.top)return;
      sessionStorage.removeItem('ps_exp_dismiss');
      if(new URL(location.href).searchParams.has('fixtureExpired'))localStorage.setItem('ps_login_expired','1');
      else localStorage.removeItem('ps_login_expired');
    },{base});
    ...
    for(const scenario of [{surface:'recovery',provider:'google'},{surface:'recovery',provider:'kakao'},{surface:'expired',provider:'google'}]){
      ...
      try{
        ...
        let button;
        if(scenario.surface==='recovery'){
          button=page.locator('.ps-oauth-rec [data-provider="'+scenario.provider+'"]');
        }else{
          await touch(page,page.locator('#expLogin'));
          await page.waitForTimeout(150);
          assert.equal(await page.locator('#psAcctWrap .acct-pop.on').count(),1,'expired login button keeps provider choice open');
          button=page.locator('#psAcctWrap .acct-pop.on').getByRole('button',{name:'Google로 계속',exact:true});
        }
        await button.waitFor({state:'visible'});
        await button.tap({trial:true});
        const rect=await geometry(button);assert.ok(rect.width>0&&rect.height>0&&rect.visibleCenter,'real touch center belongs to selected provider button '+JSON.stringify(rect));
      ...
      }catch(e){fs.writeFileSync(path.join(out,label+'-failure.json'),JSON.stringify({message:e.message,results,intercepted,errors,state:await page.evaluate(()=>({url:location.href,pop:document.querySelector('#psAcctWrap .acct-pop')?.className,banner:document.querySelector('#expBanner')?.style.display}))},null,2));await page.screenshot({path:path.join(out,label+'-failure.png')}).catch(()=>{});throw e;}
    }
